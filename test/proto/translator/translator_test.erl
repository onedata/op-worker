...
translate_file_block_from_protobuf_test() ->
    ...
    ?assertEqual(Internal, clproto_translator:translate_from_protobuf(Protobuf)),
    ?assertEqual(Internal2, clproto_translator:translate_from_protobuf(Protobuf2)).
...
translate_proxyio_request_remote_read_from_protobuf_test() ->
    ?assertEqual(#proxyio_request{
        ...
        clproto_translator:translate_from_protobuf(#'ProxyIORequest'{
            storage_id = <<"StorageID">>,
            file_id = <<"FileID">>,
...
translate_status_to_protobuf_test() ->
    ?assertEqual({status, #'Status'{code = ?OK, description = <<"It's fine">>}},
        clproto_translator:translate_to_protobuf(#status{code = ?OK, description = <<"It's fine">>})
    ),
    ?assertEqual({status, #'Status'{code = ?EIO}},
        clproto_translator:translate_to_protobuf(#status{code = ?EIO})
    ).

translate_events_to_protobuf_test() ->
    ?assertEqual({events, #'Events'{events = [#'Event'{}]}},
        clproto_translator:translate_to_protobuf(#events{events = [#event{}]})
    ).

translate_event_to_protobuf_test() ->
    ?assertEqual(#'Event'{}, clproto_translator:translate_to_protobuf(#event{})).

translate_subscription_cancellation_to_protobuf_test() ->
    ?assertEqual({subscription_cancellation, #'SubscriptionCancellation'{id = 42}},
        clproto_translator:translate_to_protobuf(#subscription_cancellation{id = <<"42">>})
    ).

translate_subscription_to_protobuf_test() ->
    ?assertEqual({subscription, #'Subscription'{id = 123}},
        clproto_translator:translate_to_protobuf(#subscription{id = <<"123">>})
    ).
...
translate_read_subscription_to_protobuf_test() ->
    ?assertEqual({file_read, #'FileReadSubscription'{
        ...
        time_threshold = 500
    }}, clproto_translator:translate_to_protobuf(#file_read_subscription{
        counter_threshold = 12,
        time_threshold = 500
    })).
...
translate_write_subscription_to_protobuf_test() ->
    ?assertEqual({file_written, #'FileWrittenSubscription'{
        counter_threshold = 12,
        time_threshold = 500
    }}, clproto_translator:translate_to_protobuf(#file_written_subscription{
        counter_threshold = 12,
        time_threshold = 500
    })).
...
translate_handshake_response_to_protobuf_test() ->
    ?assertEqual({handshake_response, #'HandshakeResponse'{
        status = 'OK'
    }}, clproto_translator:translate_to_protobuf(#handshake_response{
        status = 'OK'
    })),
    ?assertEqual({handshake_response, #'HandshakeResponse'{
...
translate_message_stream_to_protobuf_test() ->
    ?assertEqual(#'MessageStream'{stream_id = 27, sequence_number = 43412},
        clproto_translator:translate_to_protobuf(#message_stream{
...
translate_message_stream_reset_to_protobuf_test() ->
    ?assertEqual({message_stream_reset, #'MessageStreamReset'{}},
        clproto_translator:translate_to_protobuf(#message_stream_reset{})
    ).
...
translate_message_request_to_protobuf_test() ->
    ...
    }, clproto_translator:translate_to_protobuf(#message_request{
        stream_id = 94,
        lower_sequence_number = 821,
        upper_sequence_number = 196
    })).
...
translate_message_acknowledgement_to_protobuf_test() ->
    ?assertEqual({message_acknowledgement, #'MessageAcknowledgement'{
        stream_id = 44,
        sequence_number = 202}
    }, clproto_translator:translate_to_protobuf(#message_acknowledgement{
        stream_id = 44,
        sequence_number = 202
    })).

translate_pong_to_protobuf_test() ->
    ?assertEqual({pong, #'Pong'{}}, clproto_translator:translate_to_protobuf(#pong{})),
    ?assertEqual({pong, #'Pong'{data = 123}},
        clproto_translator:translate_to_protobuf(#pong{data = 123})
    ),
    ?assertEqual({pong, #'Pong'{data = "OneData"}},
        clproto_translator:translate_to_protobuf(#pong{data = "OneData"})
    ),
    ?assertEqual({pong, #'Pong'{data = some_atom}},
        clproto_translator:translate_to_protobuf(#pong{data = some_atom})
    ).
...
translate_protocol_version_to_protobuf_test() ->
    ?assertEqual({protocol_version, #'ProtocolVersion'{
        major = 3.0,
        minor = 1.1
    }}, clproto_translator:translate_to_protobuf(#protocol_version{
        major = 3.0,
        minor = 1.1
    })),
    ?assertEqual({protocol_version, #'ProtocolVersion'{
...
translate_proxyio_response_remote_data_to_protobuf_test() ->
    ?assertEqual(
        ...
        clproto_translator:translate_to_protobuf(#proxyio_response{
            status = #status{code = ?OK, description = <<"It's fine">>},
...
translate_undefined_to_protobuf_test() ->
    ?assertEqual(undefined, clproto_translator:translate_to_protobuf(undefined)).
