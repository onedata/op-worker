...
setup() ->
    ...
    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{}} end),
...
get_file_meta() ->
    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{record = #file_meta{}}} end),
    meck:expect(dao_records, get_record, fun("uuid") -> {ok, #veil_document{record = #file_meta{ctime = 1111}}} end),
...
get_descriptor() ->
    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{record = #file{}}} end),
    meck:expect(dao_records, get_record, fun("uuid") -> {ok, #veil_document{record = #file_descriptor{file = "test"}}};
...
list_descriptors() ->
    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{uuid = "uuid", record = #file{}}} end),
    meck:expect(dao_records, list_records, 2 , {ok, #view_result{rows = [#view_row{doc = #veil_document{record = #file_descriptor{file = "uuid"}}}]}}),
...
remove_file() ->
    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{uuid = "uuid", record = #file{meta_doc = "meta"}}} end),
    meck:expect(dao_records, list_records, fun (?FD_BY_FILE_VIEW, #view_query_args{skip = 0}) -> {ok, #view_result{rows = [#view_row{id = "fd", doc = #veil_document{uuid = "fd", record = #file_descriptor{file = "uuid"}}}]}};
...
get_file() ->
    ...
    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{uuid = "f", record = #file{name = "test"}}} end),
...
list_dir() ->
    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{uuid = "f", record = #file{type = ?REG_TYPE}}} end),
    ?assertThrow({dir_not_found, _}, dao_vfs:list_dir({uuid, "file"}, 1, 0)),

    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{uuid = "f", record = #file{type = ?DIR_TYPE}}} end),
...
remove_storage() ->
    ...
    meck:expect(dao_records, get_record, fun("uuid") -> {ok, #veil_document{uuid="uuid", record = #storage_info{}}} end),
...
get_storage() ->
    meck:expect(dao_records, get_record, fun(_) -> {ok, #veil_document{record = #file{}}} end),
    meck:expect(dao_records, get_record, fun("uuid") -> {ok, #veil_document{record = #storage_info{name = "test"}}};
        ("uuid2") -> {ok, #veil_document{record = #file{}}} end),
