...
fuse_group_name_test_() ->
  {foreach, fun setup/0, fun teardown/1,
    [fun save_fuse_group_name/0, fun remove_fuse_group_name/0, fun get_fuse_group_name/0]}.
...
remove_file() ->
  ...
  meck:expect(dao, list_records, fun(?FD_BY_FILE_VIEW, #view_query_args{skip = 0}) ->
    {ok, #view_result{rows = [#view_row{id = "fd", doc = #veil_document{uuid = "fd", record = #file_descriptor{file = "uuid"}}}]}};
...
save_fuse_group_name() ->
  Doc = #veil_document{record = #fuse_group_name{}},
  ?assertMatch({ok, "uuid"}, dao_vfs:save_fuse_group_name(Doc)),
  ?assertMatch({ok, "uuid"}, dao_vfs:save_fuse_group_name(#fuse_group_name{})),

  ?assertEqual(2, meck:num_calls(dao, set_db, [?SYSTEM_DB_NAME])),
  ?assertEqual(2, meck:num_calls(dao, save_record, [Doc])),
  ?assert(meck:validate([dao, dao_helper, worker_host])).

remove_fuse_group_name() ->
  meck:expect(dao, get_record, fun("uuid") -> {ok, #veil_document{uuid = "uuid", record = #fuse_group_name{}}} end),
  ?assertMatch(ok, dao_vfs:remove_fuse_group_name({uuid, "uuid"})),
  ?assert(meck:called(dao, set_db, [?SYSTEM_DB_NAME])),
  ?assert(meck:called(dao, remove_record, ["uuid"])),
  ?assert(meck:validate([dao, dao_helper, worker_host])).
...
get_fuse_group_name() ->
  meck:expect(dao, get_record, fun(_) -> {ok, #veil_document{record = #file{}}} end),
  meck:expect(dao, get_record, fun
    ("uuid") -> {ok, #veil_document{record = #fuse_group_name{name = "test"}}};
    ("uuid2") -> {ok, #veil_document{record = #file{}}}
  end),
  ?assertMatch({ok, #veil_document{record = #fuse_group_name{name = "test"}}}, dao_vfs:get_fuse_group_name({uuid, "uuid"})),
  ?assert(meck:called(dao, get_record, ["uuid"])),

  ?assertMatch({error, invalid_fuse_group_name_record}, dao_vfs:get_fuse_group_name({uuid, "uuid2"})),
  ?assert(meck:called(dao, get_record, ["uuid2"])),

  ?assertEqual(2, meck:num_calls(dao, set_db, [?SYSTEM_DB_NAME])),
  ?assert(meck:validate([dao, dao_helper])).

gen_fuse_group_name_test() ->
  StorageInfo1 = [{1, "/mnt/path1"}, {2, "/mnt/path2"}, {3, "/mnt/path3"}, {4, "/mnt/path4"}],
  StorageInfo2 = [{4, "/mnt/path4"}, {3, "/mnt/path3"}, {2, "/mnt/path2"}, {1, "/mnt/path1"}],
  StorageInfo3 = [{1, "/mnt/path4"}, {2, "/mnt/path3"}, {3, "/mnt/path2"}, {4, "/mnt/path1"}],
  {ok, Hash1} = dao_vfs:gen_fuse_group_name(StorageInfo1),
  {ok, Hash2} = dao_vfs:gen_fuse_group_name(StorageInfo2),
  {ok, Hash3} = dao_vfs:gen_fuse_group_name(StorageInfo3),

  ?assertEqual({ok, Hash1}, dao_vfs:gen_fuse_group_name(StorageInfo1)),
