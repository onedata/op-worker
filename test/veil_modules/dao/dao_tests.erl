...
save_record_test() ->
    ...
    {ok, "test_id"} = dao:save_record(#veil_document{record = ToInsert, uuid = "test_id"}),
    {ok, "test_id"} = dao:save_record(#veil_document{record = ToInsert, uuid = "test_id", force_update = true}),
...
get_record_test() ->
    ...
    meck:expect(dao_helper, name, fun(Arg) -> meck:passthrough([Arg]) end),
    {ok, #veil_document{record = #some_record{field1 = 1, field3 = true}}} = dao:get_record("test_id"),
    true = meck:validate(dao_helper),
    meck:unload(dao_helper).
