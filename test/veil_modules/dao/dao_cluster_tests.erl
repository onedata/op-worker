...
save_state() ->
    meck:expect(dao_records, save_record, fun(#veil_document{record = #some_record{}, uuid = cluster_state, force_update = true}) -> {ok, "cluster_state"} end),
    ?assertEqual({ok, "cluster_state"}, dao_cluster:save_state(#some_record{})),
    ?assert(meck:validate(dao_records)).

get_state() ->
    meck:expect(dao_records, get_record, fun(cluster_state) -> {ok, #veil_document{record = #some_record{}}} end),
...
save_fuse_session() ->
    meck:expect(dao_records, save_record, fun(#veil_document{record = #fuse_session{}, uuid = "UUID"}) -> {ok, "UUID"} end),
    ?assertEqual({ok, "UUID"}, dao_cluster:save_fuse_session(#veil_document{record = #fuse_session{}, uuid = "UUID"})),
    ?assert(meck:validate(dao_records)).

get_fuse_session() ->
    meck:expect(dao_records, get_record, fun("UUID") -> {ok, #veil_document{record = #fuse_session{uid = 123}}} end),
    ?assertMatch({ok, #veil_document{record = #fuse_session{uid = 123}}}, dao_cluster:get_fuse_session("UUID")),

    %% Cache is used
    meck:expect(dao_records, get_record, fun("UUID") -> {ok, #veil_document{record = #fuse_session{uid = 1234}}} end),
    ?assertMatch({ok, #veil_document{record = #fuse_session{uid = 123}}}, dao_cluster:get_fuse_session("UUID")),
