...
check_perms_user_file1_test() ->
  SHInfo = #storage_helper_info{name = ?SH, init_args = [?TEST_ROOT]},
  check_perms_user_file(SHInfo, ?TEST_ROOT).

check_perms_user_file2_test() ->
  SHInfo = #storage_helper_info{name = ?SH2, init_args = []},
  check_perms_user_file(SHInfo, "").

check_perms_user_file(SHInfo, TestRoot) ->
  meck:new(fslogic),

  meck:expect(fslogic, get_user_root, fun() -> {ok, "testuser"} end),
  ?assertEqual({ok, true}, storage_files_manager:check_perms(TestRoot ++ "users/testuser/somefile", SHInfo)),

  meck:expect(fslogic, get_user_root, fun() -> {ok, "testuser2"} end),
  ?assertEqual({ok, false}, storage_files_manager:check_perms(TestRoot ++ "users/testuser/somefile", SHInfo)),

  meck:expect(fslogic, get_user_root, fun() -> {error, error} end),
  storage_files_manager:check_perms(TestRoot ++ "users/testuser/somefile", SHInfo),

  ?assert(meck:validate(fslogic)),
  meck:unload(fslogic).

check_perms_read_group_file1_test() ->
  SHInfo = #storage_helper_info{name = ?SH, init_args = [?TEST_ROOT]},
  check_perms_read_group_file(SHInfo, ?TEST_ROOT).

check_perms_read_group_file2_test() ->
  SHInfo = #storage_helper_info{name = ?SH2, init_args = []},
  check_perms_read_group_file(SHInfo, "").

check_perms_read_group_file(SHInfo, TestRoot) ->
  meck:new(fslogic),
  ...
  ?assertEqual({ok, true}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo, read)),

  meck:expect(fslogic, get_user_groups, fun(_, _) -> {ok, [xyz, abc, "testgroup2", g123]} end),
  ?assertEqual({ok, false}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo, read)),

  meck:expect(fslogic, get_user_groups, fun(_, _) -> {error, error} end),
  ?assertEqual({error, can_not_get_user_groups}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo, read)),

  ?assert(meck:validate(fslogic)),
  meck:unload(fslogic).

check_perms_group_perms_file1_test() ->
  SHInfo = #storage_helper_info{name = ?SH, init_args = [?TEST_ROOT]},
  check_perms_group_perms_file(SHInfo, ?TEST_ROOT).

check_perms_group_perms_file2_test() ->
  SHInfo = #storage_helper_info{name = ?SH2, init_args = []},
  check_perms_group_perms_file(SHInfo, "").

check_perms_group_perms_file(SHInfo, TestRoot) ->
  meck:new(fslogic),
  meck:new(veilhelpers),
  ...
  ?assertEqual({ok, true}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo, perms)),

  meck:expect(fslogic, get_user_doc, fun() -> {ok, #veil_document{record = #user{login = "testuser2"}}} end),
  ?assertEqual({ok, false}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo, perms)),
  ...
  ?assertEqual({error, can_not_check_file_owner}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo, perms)),
...
check_perms_group_write_file1_test() ->
  SHInfo = #storage_helper_info{name = ?SH, init_args = [?TEST_ROOT]},
  check_perms_group_write_file(SHInfo, ?TEST_ROOT).

check_perms_group_write_file2_test() ->
  SHInfo = #storage_helper_info{name = ?SH2, init_args = []},
  check_perms_group_write_file(SHInfo, "").

check_perms_group_write_file(SHInfo, TestRoot) ->
  meck:new(fslogic),
  meck:new(veilhelpers),
  meck:new(fslogic_utils),
  ...
  ?assertEqual({ok, true}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo)),
  ...
  ?assertEqual({ok, false}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo)),

  ets:delete(EtsName),
  meck:expect(veilhelpers, exec, fun(getattr, _, _) -> {2, #st_stat{st_uid = 1000, st_mode = 8#660}} end),
  ?assertEqual({error, can_not_check_grp_perms}, storage_files_manager:check_perms(TestRoot ++ "groups/testgroup/somefile", SHInfo)),
