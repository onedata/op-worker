...
%%%--------------------------------------------------------------------
%%% @doc
%%% Unit tests for fslogic_authz module.
%%% @end
%%%--------------------------------------------------------------------
-module(fslogic_authz_test).
-author("Bartosz Walkowicz").
...
check_data_path_relation_test_() -> [
    ?_assertEqual(
        undefined,
        fslogic_authz:check_data_path_relation(
            <<"/root/binar">>,
            [<<"boot">>, <<"/root/users">>]
        )
    ),
...
is_subpath_test_() -> [
    ?_assertEqual(
        false,
        fslogic_authz:is_subpath(<<"/c/b/a">>, <<"/a/b/c">>)
    ),
    ?_assertEqual(
        false,
        fslogic_authz:is_subpath(<<"/a/b/c">>, <<"/a/b/c/">>)
    ),
    ?_assertEqual(
        true,
        fslogic_authz:is_subpath(<<"/a/b/c">>, <<"/a/b/c">>)
    ...
    ?_assertEqual(
        true,
        fslogic_authz:is_subpath(<<"/a/b/c/d">>, <<"/a/b/c">>)
    )
].
