...
get_level_0_simple_meta_test() ->
    ...
    Meta = json_metadata:find(Json, [<<"author">>]),
    ?assertEqual(<<"Pat">>, Meta).

get_level_0_integer_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    Meta = json_metadata:find(Json, [<<"version">>]),
    ?assertEqual(1, Meta).

get_level_0_list_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    Meta = json_metadata:find(Json, [<<"tags">>]),
    ?assertEqual([<<"things">>, <<"stuff">>], Meta).

get_level_0_json_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    Meta = json_metadata:find(Json, [<<"operatingsystem_support">>]),
    ?assertMatch(#{<<"operatingsystem">> := <<"RedHat">>, <<"operatingsystemrelease">> := [<<"5.0">>, <<"6.0">>]}, Meta).

get_level_1_simple_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    Meta = json_metadata:find(Json, [<<"operatingsystem_support">>, <<"operatingsystem">>]),
    ?assertEqual(<<"RedHat">>, Meta).

get_level_1_list_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    Meta = json_metadata:find(Json, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>]),
    ?assertEqual([<<"5.0">>, <<"6.0">>], Meta).

get_level_0_all_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    ?assertEqual(Json, json_metadata:find(Json, [])).

error_get_level_0_missing_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    ?assertException(throw, {error, ?ENOATTR}, json_metadata:find(Json, [<<"missing">>])).

error_get_level_1_missing_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    ?assertException(throw, {error, ?ENOATTR}, json_metadata:find(Json, [<<"operatingsystem_support">>, <<"missing">>])).

error_get_level_1_meta_from_non_container_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    ?assertException(throw, {error, ?ENOATTR}, json_metadata:find(Json, [<<"version">>, <<"missing">>])).

add_level_0_simple_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, <<"Tom">>, [<<"second_author">>]),
    ?assertEqual(Json#{<<"second_author">> => <<"Tom">>}, NewJson).

add_level_0_json_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, #{<<"first">> => <<"Tom">>, <<"second">> => <<"Pat">>}, [<<"new_author">>]),
    ?assertEqual(Json#{<<"new_author">> => #{<<"first">> => <<"Tom">>, <<"second">> => <<"Pat">>}}, NewJson).

add_level_0_list_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, [<<"Tom">>, <<"Pat">>], [<<"new_author">>]),
    ?assertEqual(Json#{<<"new_author">> => [<<"Tom">>, <<"Pat">>]}, NewJson).

override_level_0_list_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, [<<"Tom">>, <<"Pat">>], [<<"author">>]),
    ?assertEqual(Json#{<<"author">> => [<<"Tom">>, <<"Pat">>]}, NewJson).

add_level_1_json_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, <<"yum">>, [<<"operatingsystem_support">>, <<"repo_type">>]),
...
override_level_1_json_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, <<"Ubuntu">>, [<<"operatingsystem_support">>, <<"operatingsystem">>]),
    ?assertEqual(Json#{<<"operatingsystem_support">> => #{
...
merge_single_json_test() ->
    ...
    NewJson = json_metadata:merge([Json1]),
...
merge_empty_jsons_test() ->
    ...
    NewJson = json_metadata:merge([Json1, Json2]),
...
merge_three_jsons_test() ->
    ...
    NewJson = json_metadata:merge([Json1, Json2, Json3]),

    ?assertEqual(#{<<"a">> =>  #{<<"a1">> => <<"b4">>, <<"a2">> => <<"b2">>, <<"a3">> => <<"b3">>}, <<"b">> => <<"c">>, <<"d">> => <<"e">>}, NewJson).

override_list_meta_begin_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, <<"4.0">>, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>, <<"[0]">>]),
...
override_list_meta_end_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, <<"7.0">>, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>, <<"[1]">>]),
    ?assertEqual(Json#{<<"operatingsystem_support">> => #{
        <<"operatingsystem">> => <<"RedHat">>,
...
append_to_list_meta_end_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, <<"7.0">>, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>, <<"[2]">>]),
    ?assertEqual(Json#{<<"operatingsystem_support">> => #{
        <<"operatingsystem">> => <<"RedHat">>,
...
create_gap_in_list_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, <<"7.0">>, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>, <<"[4]">>]),
    ?assertEqual(Json#{<<"operatingsystem_support">> => #{
        <<"operatingsystem">> => <<"RedHat">>,
...
create_list_in_list_meta_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    NewJson = json_metadata:insert(Json, <<"7.1">>, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>, <<"[2]">>, <<"[0]">>]),
...
get_list_meta_begin_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    ?assertEqual(<<"5.0">>, json_metadata:find(Json, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>, <<"[0]">>])).

get_list_meta_end_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    ?assertEqual(<<"6.0">>, json_metadata:find(Json, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>, <<"[1]">>])).

get_list_meta_missing_test() ->
    Json = json_utils:decode(?SAMPLE_METADATA_BINARY),
    ?assertException(throw, {error, ?ENOATTR}, json_metadata:find(Json, [<<"operatingsystem_support">>, <<"operatingsystemrelease">>, <<"[2]">>])).
