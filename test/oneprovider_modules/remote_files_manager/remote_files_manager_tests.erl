...
get_helper_and_id_wrong_args_test() ->
  Storage = 1,
  Id = "id1",
  ?assertError(_, remote_files_manager:get_storage_and_id(integer_to_list(Storage) ++ Id)),

  Storage2 = "qqq",
  ?assertError(_, remote_files_manager:get_storage_and_id(filename:join(["/", Storage2, Id]))).
