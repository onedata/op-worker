    ...
    {foreach,
        ...
        [
            {"save user doc",
                fun() ->
                    ...
                    meck:expect(dao_records, save_record,
                        fun(Arg) -> 
                            case Arg of
                                #db_document{record = #user{}, uuid = ""} -> {ok, "new_uuid"};
                                #db_document{record = #user{}, uuid = UUID} -> {ok, UUID}
                            end
                        end),
                    ...
                    ?assertEqual({ok, "existing_uuid"}, dao_users:save_user(#db_document{record = #user{}, uuid = "existing_uuid"})),
            ...
            {"remove & get user doc by uuid",
                fun() ->
                    meck:expect(dao_records, get_record, fun(_UUID) -> {ok, #db_document{ record = #user{}, uuid = "existing_uuid" }} end),
                    meck:expect(dao_records, remove_record, fun(_UUID) -> ok end),
                    ?assertEqual({ok, #db_document{ record = #user{}, uuid = "existing_uuid" }}, dao_users:get_user({uuid, "existing_uuid"})),
            ...
            {"empty, duplicated or invalid view response",
                fun() ->
                    ...
                    UserDoc = #db_document{record = #user{}},
