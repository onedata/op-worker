...
get_sh_for_fuse_test() ->
  ...
  meck:expect(dao_lib, apply, fun
    (_, _, ["fuse1"], _) ->
      {ok, #db_document{record = #fuse_session{env_vars = [{testvar1, "testvalue1"}, {group_id, "group1"}]}}};
    (_, _, ["fuse2"], _) ->
      {ok, #db_document{record = #fuse_session{env_vars = [{testvar1, "testvalue1"}, {group_id, "group2"}]}}};
