...
setup() ->
    {ok, _} = rt_heap:new({local, ?TEST_HEAP}, "../", ?TEST_RT_BLOCK_SIZE).

teardown(_) ->
    ok = rt_heap:delete(?TEST_HEAP).
...
should_push_single_block() ->
    ...
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block)),
    ?assertEqual({ok, Block}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_push_blocks_from_different_files() ->
    ...
    ?assertEqual({ok, Block1}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block2}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_push_many_blocks() ->
    ...
    lists:foreach(fun(N) ->
        ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block#rt_block{offset = N * ?TEST_RT_BLOCK_SIZE}))
    end, lists:seq(0, BlocksAmount)),
    lists:foreach(fun(N) ->
        ?assertEqual({ok, Block#rt_block{offset = N * ?TEST_RT_BLOCK_SIZE}}, rt_heap:fetch(?TEST_HEAP))
    end, lists:seq(0, BlocksAmount)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_merge_small_blocks() ->
    ...
    lists:foreach(fun(N) ->
        ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block#rt_block{offset = N}))
    end, lists:seq(0, ?TEST_RT_BLOCK_SIZE - 1)),
    ?assertEqual({ok, Block#rt_block{offset = 0, size = ?TEST_RT_BLOCK_SIZE}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_split_large_block() ->
    ...
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block)),
    lists:foreach(fun(N) ->
        ?assertEqual({ok, Block#rt_block{offset = N * ?TEST_RT_BLOCK_SIZE, size = ?TEST_RT_BLOCK_SIZE}}, rt_heap:fetch(?TEST_HEAP))
    end, lists:seq(0, FullBlocksAmount - 1)),
    ?assertEqual({ok, Block#rt_block{offset = FullBlocksAmount * ?TEST_RT_BLOCK_SIZE, size = LastBlockSize}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_change_priority_1() ->
    ...
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block2)),
    ?assertEqual({ok, Block2}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block1#rt_block{size = 3}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block1#rt_block{offset = 6, size = 4}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)),

    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block2)),
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block1)),
    ?assertEqual({ok, Block2#rt_block{priority = 2}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block1#rt_block{size = 3}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block1#rt_block{offset = 6, size = 4}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_change_priority_2() ->
    ...
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block1)),
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block2)),
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block3)),
    ?assertEqual({ok, Block2#rt_block{size = 5}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block1#rt_block{size = 2}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_change_priority_3() ->
    ...
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block2)),
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block3)),
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block4)),
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block5)),
    ?assertEqual({ok, Block5}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block4#rt_block{size = 1}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block4#rt_block{offset = 6, size = 1}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block3#rt_block{size = 1}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block3#rt_block{offset = 7, size = 1}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block2#rt_block{size = 1}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block2#rt_block{offset = 8, size = 1}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block1#rt_block{size = 1}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({ok, Block1#rt_block{offset = 9, size = 1}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_change_priority_4() ->
    ...
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block#rt_block{offset = 0, size = ?TEST_RT_BLOCK_SIZE})),
    ?assertEqual({ok, Block#rt_block{offset = 0, size = ?TEST_RT_BLOCK_SIZE}}, rt_heap:fetch(?TEST_HEAP)),
    ?assertEqual({error, "Empty heap"}, rt_heap:fetch(?TEST_HEAP)).
...
should_change_priority_5() ->
    ...
    ?assertEqual(ok, rt_heap:push(?TEST_HEAP, Block1#rt_block{priority = 2})),
