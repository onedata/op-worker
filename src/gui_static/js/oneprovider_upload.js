...
// Must be the same as macros in oneprovider_upload.erl
...
oneprovider_send_pending_files = function (form, input) {
    var file = null;
        ...
        $(input).fileupload({
            ...
            fail: function (e, data, options) {
                $(findTagByFilename(data.files[0].name.split('\\').join(''))).
                    ...
                    attr("status", "error");
                oneprovider_increment_pending_upload_counter(form, -1);
            },
            add: function (e, data) {
                $(".dummy_tag").remove();
