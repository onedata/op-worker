...
spaces_table_collapsed(SpacesDetails) ->
    TableName = <<"Spaces">>,
    NavigationBody = vcn_gui_utils:expand_button(<<"Expand All">>, {message, expand_spaces_table}),
...
spaces_table_expanded(SpacesDetails) ->
    TableName = <<"Spaces">>,
    NavigationBody = vcn_gui_utils:collapse_button(<<"Collapse All">>, {message, collapse_spaces_table}),
...
space_row_collapsed(RowId, #space_details{id = SpaceId, name = SpaceName} = SpaceDetails) ->
    [
        ...
        #td{
            style = ?NAVIGATION_COLUMN_STYLE,
            body = vcn_gui_utils:expand_button({message, {expand_space_row, RowId, SpaceDetails}})
        }
    ].
...
space_row_expanded(RowId, #space_details{id = SpaceId, name = SpaceName} = SpaceDetails) ->
    ...
    [
        ...
        #td{
            style = ?NAVIGATION_COLUMN_STYLE,
            body = vcn_gui_utils:collapse_button({message, {collapse_space_row, RowId, SpaceDetails}})
        }
    ].
            ...
            receive
                {create_space, Name} ->
                    NewState = try
                                   ...
                                   gr_adapter:synchronize_user_spaces({vcn_gui_utils:get_global_user_id(), AccessToken}),
...
event(init) ->
    try
        ...
        _:Reason ->
            ...
            vcn_gui_utils:message(<<"error_message">>, <<"Cannot fetch Spaces.<br>Please try again later.">>)
    end;
