	...
	case cowboy_req:header(<<"upgrade">>, Req) of
		...
		{Bin, Req2} when is_binary(Bin) ->
			case cowboy_bstr:to_lower(Bin) of
				...
				_Any ->
					{ok, Req3} = veil_cowboy_bridge:apply(cowboy_req, reply, [501, [], [], Req2]),
...
init(_Transport, Req, _Opts, _Method) ->
	{ok, Req2} = veil_cowboy_bridge:apply(cowboy_req,reply,[405, [], [], Req]),
...
handle(Req, State, <<"OPTIONS">>)->
       ...
       {ok, Req1} = veil_cowboy_bridge:apply(cowboy_req,reply,[200, Headers, [], cowboy_req:compact(Req)]),
       {ok, Req1, State};
handle(Req, State=#state{handler=Handler, handler_state=HandlerState},
		<<"POST">>) ->
	case veil_cowboy_bridge:apply(cowboy_req,body,[Req]) of
		{ok, Data, Req2} ->
			case Handler:stream(Data, Req2, HandlerState) of
				...
				{reply, Reply, Req3, HandlerState2} ->
					{ok, Req4} = veil_cowboy_bridge:apply(cowboy_req,reply,[200, [], Reply, Req3]),
...
info(Message, Req,
	...
	case Handler:info(Message, Req, HandlerState) of
		...
		{reply, Data, Req2, HandlerState2} ->
			{ok, Req3} = veil_cowboy_bridge:apply(cowboy_req,reply,[200, [], Data, Req2]),
			{ok, Req3, State#state{handler_state=HandlerState2}}
	end.
