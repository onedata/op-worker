...
handle(_ProtocolVersion, LogMessage) when is_record(LogMessage, logmessage) ->
    ...
    dispatch_client_log(Message, Timestamp, client_loglevel_int_to_atom(SeverityAsInt), Metadata, User),
    ok;
...
%% from this or external node and calls do_log()
%% @end
-spec dispatch_client_log(Message :: string(), Timestamp :: term(), Severity :: atom(), Metadata :: list(), User :: string()) -> Result when
    Result :: ok.
%% ====================================================================
dispatch_client_log(Message, Timestamp, Severity, Metadata, User) ->
    try
        % Send log to subscribers
        lists:foreach(
            fun(Sub) ->
                Sub ! {log, {Message, Timestamp, Severity, Metadata, User}}
