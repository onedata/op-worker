...
is_stream_message(#client_message{} = Msg) ->
    SessId = router:get_session_id(Msg),
...
is_stream_message(#server_message{message_stream = undefined}) ->
    false;
is_stream_message(#server_message{session_id = SessId}) ->
    case session_utils:is_provider_session_id(SessId) of
        true ->
            ignore;
        false ->
...
route_message(#client_message{session_id = From} = Msg) ->
    case session_utils:is_provider_session_id(From) of
        true -> ok;
        false -> sequencer:communicate_with_sequencer_manager(Msg, From)
    end;
route_message(#server_message{session_id = Ref} = Msg) ->
