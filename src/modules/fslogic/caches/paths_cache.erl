...
init(all) ->
    try provider_logic:get_spaces() of
        ...
        ?ERR_NO_CONNECTION_TO_ONEZONE(_) = ErrorNoConnectionToOnezone ->
            ?debug("Unable to initialize paths caches due to: ~tp", [ErrorNoConnectionToOnezone]);
        ?ERR_UNREGISTERED_ONEPROVIDER = ErrorUnregisteredOneprovider ->
            ?debug("Unable to initialize paths caches due to: ~tp", [ErrorUnregisteredOneprovider]);
        Error = {error, _} ->
