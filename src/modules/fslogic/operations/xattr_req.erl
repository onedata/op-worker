...
%% @end
%%--------------------------------------------------------------------
-spec get_xattr(fslogic_context:ctx(), file_info:file_info(), xattr:name(), Inherited :: boolean()) ->
...
%% @end
%%--------------------------------------------------------------------
-spec set_xattr(fslogic_context:ctx(), file_info:file_info(), #xattr{}) ->
    fslogic_worker:provider_response().
...
%% @end
%%--------------------------------------------------------------------
-spec remove_xattr(fslogic_context:ctx(), file_info:file_info(), xattr:name()) ->
    fslogic_worker:provider_response().
...
remove_xattr(Ctx, File, XattrName) ->
    ...
    case xattr:delete_by_name(FileUuid, XattrName) of %todo pass file_info
        ok ->
...
%% @end
%%--------------------------------------------------------------------
-spec list_xattr(fslogic_context:ctx(), file_info:file_info(), Inherited :: boolean(), ShowInternal :: boolean()) ->
    fslogic_worker:provider_response().
...
%% @end
%%--------------------------------------------------------------------
-spec get_custom_xattr(fslogic_context:ctx(), file_info:file_info(), xattr:name(), Inherited :: boolean()) ->
    fslogic_worker:provider_response().
-check_permissions([{traverse_ancestors, 2}, {?read_metadata, 2}]).
get_custom_xattr(_Ctx, File, XattrName, Inherited) ->
    {{uuid, FileUuid} , _File2} = file_info:get_uuid_entry(File),
    case xattr:get_by_name(FileUuid, XattrName, Inherited) of %todo pass file_info
        {ok, XattrValue} ->
...
%% @end
%%--------------------------------------------------------------------
-spec set_custom_xattr(fslogic_context:ctx(), file_info:file_info(), #xattr{}) ->
    fslogic_worker:provider_response().
-check_permissions([{traverse_ancestors, 2}, {?write_metadata, 2}]).
set_custom_xattr(Ctx, File, #xattr{name = XattrName, value = XattrValue}) ->
    {{uuid, FileUuid} , _File2} = file_info:get_uuid_entry(File),
    case xattr:save(FileUuid, XattrName, XattrValue) of %todo pass file_info
        {ok, _} ->
            fslogic_times:update_ctime({uuid, FileUuid}, fslogic_context:get_user_id(Ctx)), %todo pass file_info
            #provider_response{status = #status{code = ?OK}};
        {error, {not_found, custom_metadata}} ->
