...
new(SessId) ->
    case session:get(SessId) of
        {ok, #document{value = #session{type = rest, last_access_timer = LastAccessTimer}} = Session} ->
            {ok, TTL} = application:get_env(?APP_NAME, rest_session_grace_period_seconds),
            % TODO VFS-6586 - refactor rest session expiration
            {ok, UpdatedSession} = case stopwatch:read_seconds(LastAccessTimer) > 0.6 * TTL of
                true ->
        ...
        {ok, #document{value = #session{type = gui, last_access_timer = LastAccessTimer}} = Session} ->
