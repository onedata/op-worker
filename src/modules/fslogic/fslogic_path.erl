...
%% API
-export([get_full_file_name/2, verify_file_name/1]).
...
%% @end
-spec split(Path :: file:path()) -> [binary()].
%%--------------------------------------------------------------------
split(Path) ->
...
%% @end
%%--------------------------------------------------------------------
-spec get_full_file_name(fslogic:ctx(), FileName :: file:path()) ->
    {ok, file:path()} | {error, Reason :: any()}.
...
%% @end
%%--------------------------------------------------------------------
-spec verify_file_name(FileName :: file:path()) -> Result when
...
-spec basename(Path :: file:path()) -> file:path().
%% ==================================================================
basename(Path) ->
...
%%--------------------------------------------------------------------
-spec is_space_dir(Path :: file:path()) -> boolean().
is_space_dir(Path) ->
    case split(Path) of
        [] -> true;
...
%% @end
%%--------------------------------------------------------------------
-spec ensure_path_begins_with_slash(Path :: file:path()) -> file:path().
