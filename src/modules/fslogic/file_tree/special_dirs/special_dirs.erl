...
-export([report_new_space/1, report_new_user/1]).
...
-define(RESTRICTED_FOR_HARVESTING, [
    trash,
    archives_root_dir,
    dataset_archives_root_dir,
    archive_dir
...
-define(RESTRICTED_FOR_DATASETS, [
    global_root_dir,
    user_root_dir,
    trash,
    tmp_dir,
...
-define(SCOPE_ROOT_DIRS, [
    global_root_dir,
    user_root_dir
]).

-define(IGNORED_IN_EVENTS, [
    trash,
    tmp_dir,
    opened_deleted_files_dir
]).

-define(WITHOUT_PARENT, [
    global_root_dir,
    user_root_dir,
    trash,
    tmp_dir,
...
-spec report_new_space(od_space:id()) -> ok.
report_new_space(SpaceId) ->
...
is_special(Uuid) ->
    is_included_in(?ALL_SPECIAL_DIRS, Uuid).
...
is_operation_allowed(Uuid, Operation) ->
    case extract_special_module(Uuid) of
        {true, DirType} ->
            DirType:is_operation_allowed(Operation);
        undefined ->
            not_special
    end.
...
is_scope_root_dir(Uuid) ->
    is_included_in(?SCOPE_ROOT_DIRS, Uuid).


-spec is_restricted_for_datasets(file_meta:uuid()) -> boolean().
is_restricted_for_datasets(Uuid) ->
    is_included_in(?RESTRICTED_FOR_DATASETS, Uuid).


-spec is_harvested(file_meta:uuid()) -> boolean().
is_harvested(Uuid) ->
    not is_included_in(?RESTRICTED_FOR_HARVESTING, Uuid).


-spec is_ignored_in_dir_stats(file_meta:uuid()) -> boolean().
is_ignored_in_dir_stats(Uuid) ->
    is_included_in(?IGNORED_IN_DIR_STATS, Uuid).


-spec is_ignored_in_events(file_meta:uuid()) -> boolean().
is_ignored_in_events(Uuid) ->
    is_included_in(?IGNORED_IN_EVENTS, Uuid).


-spec is_without_parent(file_meta:uuid()) -> boolean().
is_without_parent(Uuid) ->
    is_included_in(?WITHOUT_PARENT, Uuid).


-spec get_times_if_special(file_meta:uuid(), [times_api:times_type()]) -> {true, times:record()} | not_special.
get_times_if_special(Uuid, RequestedTimes) ->
    case extract_special_module(Uuid) of
        {true, Module} ->
            case erlang:function_exported(Module, get_times, 2) of
                ...
                false ->
                    % dirs not exporting get_times/2 can be handled as not special dirs
                    not_special
            end;
        undefined ->
            not_special
...
%% @private
-spec is_included_in([module()], file_meta:uuid()) -> boolean().
is_included_in(Modules, Uuid) ->
    lists:any(fun(DirType) ->
        DirType:is_special(uuid, Uuid)
    end, Modules).
