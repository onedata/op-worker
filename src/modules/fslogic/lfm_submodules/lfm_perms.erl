...
get_acl(SessId, FileKey) ->
    CTX = fslogic_context:new(SessId),
    {guid, GUID} = fslogic_uuid:ensure_guid(CTX, FileKey),
    lfm_utils:call_fslogic(SessId, provider_request, {guid, GUID}, #get_acl{},
        fun(#acl{value = Json}) ->
...
set_acl(SessId, FileKey, Acl) ->
    ...
    lfm_utils:call_fslogic(SessId, provider_request, {guid, GUID},
...
remove_acl(SessId, FileKey) ->
    CTX = fslogic_context:new(SessId),
    {guid, GUID} = fslogic_uuid:ensure_guid(CTX, FileKey),
    lfm_utils:call_fslogic(SessId, provider_request, {guid, GUID}, #remove_acl{},
        fun(_) -> ok end).
