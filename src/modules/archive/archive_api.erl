...
-spec cancel_archivisation(archive:doc() | archive:id(), archive:cancel_preservation_policy()) -> 
    ok | {error, term()}.
...
cancel_archivisation(ArchiveId, PreservationPolicy) ->
    case archive:get(ArchiveId) of
        {ok, ArchiveDoc} -> cancel_archivisation(ArchiveDoc, PreservationPolicy);
        ?ERROR_NOT_FOUND -> ok;
        {error, _} = Error -> Error
    end.
