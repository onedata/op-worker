...
%% API
-export([get_by_name/2, delete_by_name/2, exists_by_name/2, save/3, list/1]).
...
get_by_name(FileUuid, XattrName) ->
    custom_metadata:get_xattr_metadata(FileUuid, XattrName).
...
list(FileUuid) ->
    custom_metadata:list_xattr_metadata(FileUuid).