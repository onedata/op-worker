...
-export([
    ...
    detach/2, reattach/2,
    get_attrs/2,
    list_space/3, list_dataset/3
]).

-type attrs() :: #dataset_attrs{}.
...
-spec get_attrs(session:id(), dataset:id()) ->
    {ok, attrs()} | lfm:error_reply().
get_attrs(SessId, DatasetId) ->
    SpaceGuid = get_space_guid(DatasetId),
    remote_utils:call_fslogic(SessId, provider_request, SpaceGuid,
        #get_dataset_attrs{id = DatasetId},
        fun(#dataset_attrs{} = Attrs) -> {ok, Attrs} end).


-spec list_space(session:id(), od_space:id(), datasets_structure:opts()) ->
    ok | lfm:error_reply().
list_space(SessId, SpaceId, Opts) ->
    SpaceGuid = fslogic_uuid:spaceid_to_space_dir_guid(SpaceId),
    remote_utils:call_fslogic(SessId, provider_request, SpaceGuid,
        #list_space_datasets{opts = Opts},
