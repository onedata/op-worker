...
get_routing_key(#file_attr_changed_event{file_attr = FileAttr}, RoutingBase) ->
    get_parent_connected_routing_key(<<"file_attr_changed.">>, FileAttr#file_attr.guid, RoutingBase);
...
get_routing_key(#file_removed_event{file_guid = FileGuid}, RoutingBase) ->
    get_parent_connected_routing_key(<<"file_removed.">>, FileGuid, RoutingBase);
get_routing_key(#file_renamed_event{top_entry = Entry}, RoutingBase) ->
    get_parent_connected_routing_key(<<"file_renamed.">>, Entry#file_renamed_entry.old_guid, RoutingBase);
...
%% @end
%%--------------------------------------------------------------------
-spec get_parent_connected_routing_key(binary(), fslogic_worker:file_guid(), routing_info()) ->
    {ok, Key :: subscription_manager:key()} |
    {ok, Key :: subscription_manager:key(), SpaceIDFilter :: od_space:id()} | {error, session_only}.
get_parent_connected_routing_key(Base, FileGuid, #{file_ctx := FileCtx, key_base := KeyBase}) ->
...
get_parent_connected_routing_key(Base, FileGuid, #{file_ctx := FileCtx}) ->
    {ParentGuid, _} = file_ctx:get_parent_guid(FileCtx, undefined),
    get_parent_connected_routing_key(Base, FileGuid, #{file_ctx => FileCtx, key_base => ParentGuid});
get_parent_connected_routing_key(Base, FileGuid, #{key_base := KeyBase}) ->
    FileCtx = file_ctx:new_by_guid(FileGuid),
    get_parent_connected_routing_key(Base, FileGuid, #{file_ctx => FileCtx, key_base => KeyBase});
get_parent_connected_routing_key(Base, FileGuid, _) ->
    FileCtx = file_ctx:new_by_guid(FileGuid),
    {ParentGuid, _} = file_ctx:get_parent_guid(FileCtx, undefined),
    get_parent_connected_routing_key(Base, FileGuid, #{file_ctx => FileCtx, key_base => ParentGuid}).