...
get_user_ctx(SessionId, UserId, SpaceId, StorageRecord, Helper) ->
    #luma_config{url = LumaUrl} = LumaConfig = storage:get_luma_config(StorageRecord),
    Url = str_utils:format_bin("~s/map_user_credentials", [LumaUrl]),
    ReqHeaders = get_request_headers(LumaConfig),
    ReqBody = get_request_body(SessionId, UserId, SpaceId, StorageRecord),
...
get_group_ctx(_GroupId, _SpaceId, _StorageRecord, #helper{name = ?CEPH_HELPER_NAME}) ->
    undefined;
get_group_ctx(_GroupId, _SpaceId, _StorageRecord, #helper{name = ?CEPHRADOS_HELPER_NAME}) ->
    undefined;
get_group_ctx(_GroupId, _SpaceId, _StorageRecord, #helper{name = ?S3_HELPER_NAME}) ->
    undefined;
get_group_ctx(_GroupId, _SpaceId, _StorageRecord, #helper{name = ?SWIFT_HELPER_NAME}) ->
    undefined;
get_group_ctx(GroupId, SpaceId, StorageRecord, Helper) ->
    ...
    ReqHeaders = get_request_headers(LumaConfig),
    ReqBody = get_group_request_body(GroupId, SpaceId, StorageRecord),
    case luma_proxy:http_client_post(Url, ReqHeaders, ReqBody) of
        {ok, 200, _RespHeaders, RespBody} ->
...
-spec get_request_body(session:id(), od_user:id(), od_space:id(), storage:record()) ->
    Body :: binary().
get_request_body(SessionId, UserId, SpaceId, StorageRecord) ->
...
-spec get_group_request_body(od_group:id() | undefined, od_space:id(), storage:record()) ->
    Body :: binary().
get_group_request_body(undefined, SpaceId, StorageRecord) ->
    StorageId = storage:get_id(StorageRecord),
    StorageName = storage:get_name(StorageRecord),
    Body = #{
        <<"spaceId">> => SpaceId,
    ...
    json_utils:encode(filter_null_and_undefined_values(Body));
get_group_request_body(GroupId, SpaceId, StorageRecord) ->
    StorageId = storage:get_id(StorageRecord),
    StorageName = storage:get_name(StorageRecord),
    Body = #{
        <<"groupId">> => GroupId,
