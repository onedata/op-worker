...
%%%  * AUTO_FEED - LUMA mappings are automatically resolved and LUMA DB is unaffected.
...
%%% Reverse LUMA API
%%%-------------------------------------------------------------------
%%% This API is used only by storage import mechanism.
...
map_to_display_credentials(OwnerId, SpaceId, Storage) ->
    try
        ...
        Error:Reason:Stacktrace ->
            ?error_stacktrace(
                "luma:map_to_display_credentials for user: ~tp on storage: ~tp failed with unexpected error ~tp:~tp.",
...
map_to_storage_credentials_internal(UserId, SpaceId, Storage) ->
    try
        ...
        Error2:Reason:Stacktrace ->
            ?error_stacktrace(
                "luma:map_to_storage_credentials for user: ~tp on storage: ~tp failed with unexpected error ~tp:~tp.",
