...
%% API functions
-export([invalidate/1, invalidate/2]).
...
map_to_storage_credentials(?ROOT_SESS_ID, ?ROOT_USER_ID, _SpaceId, Storage) ->
    Helper = storage:get_helper(Storage),
    {ok, helper:get_admin_ctx(Helper)};
map_to_storage_credentials(SessId, UserId, SpaceId, StorageId) when is_binary(StorageId)->
    {ok, Storage} = storage:get(StorageId),
    map_to_storage_credentials(SessId, UserId, SpaceId, Storage);
map_to_storage_credentials(SessionId, UserId, SpaceId, Storage) ->
    % todo tutaj trzeba wychwycic SpaceOwnerId !!!!
    try
        case luma_users_cache:get(Storage, UserId, SessionId) of
            {ok, LumaUser} ->
...
map_to_display_credentials(OwnerId, SpaceId, Storage) ->
    % todo tutaj trzeba wychwycic SpaceOwnerId !!!! i root?
...
%% @end
%%--------------------------------------------------------------------
-spec map_uid_to_onedata_user(uid(), od_space:id(), storage:id() | storage:data()) ->
    {ok, od_user:id()} | {error, term()}.
map_uid_to_onedata_user(Uid, SpaceId, StorageId) when is_binary(StorageId) ->
    {ok, Storage} = storage:get(StorageId),
    map_uid_to_onedata_user(Uid, SpaceId, Storage);
map_uid_to_onedata_user(Uid, SpaceId, Storage) ->
...
%% @end
%%--------------------------------------------------------------------
-spec map_acl_user_to_onedata_user(acl_who(), storage:id() | storage:data()) ->
    {ok, od_user:id()} | {error, term()}.
map_acl_user_to_onedata_user(AclUser, StorageId) when is_binary(StorageId) ->
    {ok, Storage} = storage:get(StorageId),
...
%% @end
%%--------------------------------------------------------------------
-spec map_acl_group_to_onedata_group(acl_who(), storage:id() | storage:data()) ->
    {ok, od_group:id()} | {error, term()}.
map_acl_group_to_onedata_group(AclGroup, StorageId) when is_binary(StorageId) ->
    {ok, Storage} = storage:get(StorageId),
