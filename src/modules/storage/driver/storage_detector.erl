...
-spec run_diagnostics(all_nodes | this_node, helper_config:t(), luma_config:feed(), diagnostic_opts()) ->
    ok | {errors:error(), diagnostic_error_details()}.
run_diagnostics(all_nodes, HelperConfig, LumaFeed, Opts) ->
    Nodes = consistent_hashing:get_all_nodes(),
    run_diagnostics_on_nodes(Nodes, HelperConfig, LumaFeed, Opts);
run_diagnostics(this_node, HelperConfig, LumaFeed, Opts) ->
    run_diagnostics_on_nodes([node()], HelperConfig, LumaFeed, Opts).


-spec check_storage_access(helper_config:t(), helper_config:user_ctx()) ->
    ok | {error, term()}.
check_storage_access(HelperConfig, UserCtx) ->
    Handle = helpers:get_helper_handle(HelperConfig, UserCtx),
    ok = helpers:check_storage_availability(Handle).


-spec create_test_file(helper_config:t(), helper_config:user_ctx()) ->
    {ok, helpers:file_id()}.
create_test_file(#helper_config{name = ?S3_HELPER_NAME, args = #{<<"archiveStorage">> := <<"true">>}} = HelperConfig, UserCtx) ->
    ...
    create_test_file(HelperConfig, UserCtx, ?DUMMY_SPACE_DIR_NAME);
create_test_file(HelperConfig, UserCtx) ->
    create_test_file(HelperConfig, UserCtx, <<>>).


-spec create_test_file(helper_config:t(), helper_config:user_ctx(), binary()) ->
    {ok, helpers:file_id()}.
create_test_file(HelperConfig, UserCtx, SpaceDirName) ->
...
-spec write_test_file(helper_config:t(), helper_config:user_ctx(), helpers:file_id()) ->
    {ok, binary()}.
write_test_file(HelperConfig, UserCtx, FileId) ->
    Handle = helpers:get_helper_handle(HelperConfig, UserCtx),
...
-spec check_test_file_content(helper_config:t(), helper_config:user_ctx(), helpers:file_id(), binary()) ->
    ok.
check_test_file_content(HelperConfig, UserCtx, FileId, ExpectedContent) ->
    {ok, ExpectedContent} = read_test_file(HelperConfig, UserCtx, FileId),
    ok.


-spec read_test_file(helper_config:t(), helper_config:user_ctx(), helpers:file_id()) ->
    {ok, binary()}.
read_test_file(HelperConfig, UserCtx, FileId) ->
    Handle = helpers:get_helper_handle(HelperConfig, UserCtx),
...
-spec remove_test_file(helper_config:t(), helper_config:user_ctx(), helpers:file_id(),
    Size :: non_neg_integer()) -> ok | no_return().
remove_test_file(HelperConfig, UserCtx, FileId, Size) ->
    Handle = helpers:get_helper_handle(HelperConfig, UserCtx),
...
%% @private
-spec run_diagnostics_on_nodes([node()], helper_config:t(), luma_config:feed(), diagnostic_opts()) ->
    ok | {errors:error(), diagnostic_error_details()}.
run_diagnostics_on_nodes(_Nodes, #helper_config{name = ?NULL_DEVICE_HELPER_NAME}, _LumaFeed, _) ->
    ok;
run_diagnostics_on_nodes(_Nodes, #helper_config{name = ?HTTP_HELPER_NAME}, _LumaFeed, _) ->
    ok;
run_diagnostics_on_nodes(Nodes, HelperConfig, LumaFeed, Options) ->
    try
        case ?SKIP_STORAGE_DETECTION of
            ...
            false ->
                ...
                {ok, ExtendedAdminCtx} = luma:add_helper_specific_fields(
                    ?ROOT_USER_ID, ?ROOT_SESS_ID, AdminCtx, HelperConfig, LumaFeed
                ),
                run_diagnostics_on_nodes_insecure(Nodes, HelperConfig, ExtendedAdminCtx, Options)
        end
...
%% @private
-spec run_diagnostics_on_nodes_insecure([node()], helper_config:t(), helper_config:user_ctx(), diagnostic_opts()) ->
    ok.
run_diagnostics_on_nodes_insecure(Nodes, HelperConfig, UserCtx, Opts) ->
    BasicArgList = [[HelperConfig, UserCtx] || _N <- Nodes],
