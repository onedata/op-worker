...
init(DataSpec, #{<<"end">> := EndNum} = InitialArgs) when is_integer(EndNum) ->
    StartNum = maps:get(<<"start">>, InitialArgs, 0),
    Step = maps:get(<<"step">>, InitialArgs, 1),

    case
        atm_data_spec:get_type(DataSpec) =:= atm_integer_type andalso
...
init(_DataSpec, _InitialArgs) ->
    throw(?EINVAL).


-spec get_data_spec(container()) -> atm_data_spec:spec().
get_data_spec(#atm_range_data_container{data_spec = DataSpec}) ->
    DataSpec.
...
to_json(#atm_range_data_container{
    ...
    #{
        <<"dataSpec">> => atm_data_spec:to_json(DataSpec),
        <<"start">> => StartNum,
        <<"end">> => EndNum,
        <<"step">> => Step
    }.
...
from_json(#{
    <<"dataSpec">> := DataSpecJson,
    <<"start">> := StartNum,
    <<"end">> := EndNum,
    <<"step">> := Step
}) ->
    #atm_range_data_container{
        data_spec = atm_data_spec:from_json(DataSpecJson),
        start_num = StartNum,
        end_num = EndNum,
        step = Step
    }.
