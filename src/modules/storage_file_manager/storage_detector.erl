...
%% API
-export([generate_file_id/0, create_test_file/2, create_test_file/3,
    read_test_file/2, read_test_file/3, update_test_file/2, update_test_file/3,
    remove_test_file/2, remove_test_file/3]).
...
%% @end
%%--------------------------------------------------------------------
-spec create_test_file(helpers:helper_handle(), helpers:file_id()) ->
    Content :: binary().
create_test_file(Handle, FileId) ->
...
%% @end
%%--------------------------------------------------------------------
-spec read_test_file(helpers:helper_handle(), helpers:file_id()) ->
    Content :: binary().
read_test_file(Handle, FileId) ->
...
update_test_file(Helper, UserCtx, FileId) ->
    Handle = helpers:get_helper_handle(Helper, UserCtx),
    update_test_file(Handle, FileId).
...
%% @end
%%--------------------------------------------------------------------
-spec update_test_file(helpers:helper_handle(), helpers:file_id()) ->
    Content :: binary().
update_test_file(Handle, FileId) ->
    Content = fslogic_utils:random_ascii_lowercase_sequence(?TEST_FILE_LEN),
    {ok, FileHandle} = helpers:open(Handle, FileId, write),
...
%% @end
%%--------------------------------------------------------------------
-spec remove_test_file(helpers:helper_handle(), helpers:file_id()) ->
    ok.
remove_test_file(Handle, FileId) ->
    Noop = fun(_) -> ok end,
