...
-spec get_posix_user_ctx(od_user:id(), undefined | od_group:id(), od_space:id()) ->
    posix_user_ctx().
get_posix_user_ctx(UserId, GroupId, SpaceId) ->
    {ok, UserCtx} = case select_posix_storage(SpaceId) of
        {ok, StorageDoc} ->
...
%% @end
%%--------------------------------------------------------------------
-spec fetch_user_ctx(od_user:id(), od_group:id() | undefined, od_space:id(), storage:doc(),
    storage:helper()) -> {ok, user_ctx()} | {error, Reason :: term()} | undefined.
fetch_user_ctx(UserId, GroupId, SpaceId, StorageDoc, Helper) ->
