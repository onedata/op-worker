...
prepare([<<"parentID">> | Tail], #{path := Path, auth := Auth} = State) ->
    {ok, #file_attr{guid = Guid}} = logical_file_manager:stat(Auth, {path, filepath_utils:parent_dir(Path)}),
    {ok, Id} = file_id:guid_to_objectid(Guid),
...
prepare([<<"childrenrange">> | Tail], #{options := Opts, guid := Guid, auth := Auth} = State) ->
    {ok, ChildNum} = logical_file_manager:get_children_count(Auth, {guid, Guid}),
    {From, To} =
...
prepare([{<<"children">>, From, To} | Tail], #{guid := Guid, auth := Auth} = State) ->
    ...
    {ok, List} = logical_file_manager:ls(Auth, {guid, Guid}, From1, To1 - From1 + 1),
    Children = lists:map(
...
prepare([<<"children">> | Tail], #{guid := Guid, auth := Auth} = State) ->
    {ok, MaxChildren} = application:get_env(?APP_NAME, max_children_per_request),
    {ok, List} = logical_file_manager:ls(Auth, {guid, Guid}, 0, MaxChildren + 1),
...
distinguish_files(Guid, Name, Auth) ->
    case logical_file_manager:stat(Auth, {guid, Guid}) of
