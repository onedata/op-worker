...
%% @end
%%--------------------------------------------------------------------
-spec stream_binary(HttpStatus :: non_neg_integer(), req(),
...
stream_binary(HttpStatus, Req, #{path := Path, auth := Auth} = State, Size, Ranges) ->
    ...
    {ok, FileHandle} = logical_file_manager:open(Auth, {path, Path} ,read),
...
%% @end
%%--------------------------------------------------------------------
-spec stream_cdmi(req(), State ::maps:map(), Size :: non_neg_integer(),
...
-spec stream_range(req(), State :: maps:map(), Range, Encoding :: binary(),
  BufferSize :: integer(), FileHandle :: logical_file_manager:handle()) -> Result when
...
stream_range(Req, State, {From, To}, Encoding, BufferSize, FileHandle) ->
    ...
    {ok, NewFileHandle, Data} = logical_file_manager:read(FileHandle, From, ReadBufSize),
    DataSize = size(Data),
    case DataSize of
        0 ->
           ok;
...
%% @end
%%--------------------------------------------------------------------
-spec write_body_to_file(req(), integer(), logical_file_manager:handle()) ->
    {ok, req()}.
write_body_to_file(Req0, Offset, FileHandle) ->
    WriteFun = fun Write(Req, WriteOffset, Handle) ->
        ...
        {ok, _NewHandle, Bytes} = logical_file_manager:write(Handle, WriteOffset, Chunk),
        case Status of
