...
%% @end
%%--------------------------------------------------------------------
-spec get_user_metadata(rest_auth:auth(), logical_file_manager:file_key()) ->
    maps:map().
get_user_metadata(Auth, FileKey) ->
    {ok, Names} = logical_file_manager:list_xattr(Auth, FileKey, false, true),
    Metadata = lists:filtermap(
        fun
            (<<?USER_METADATA_FORBIDDEN_PREFIX_STRING, _/binary>>) -> false;
            (Name) ->
                case logical_file_manager:get_xattr(Auth, FileKey, Name, false) of
...
%%--------------------------------------------------------------------
-spec update_user_metadata(rest_auth:auth(), logical_file_manager:file_key(),
    maps:map()) -> ok.
...
update_user_metadata(Auth, FileKey, UserMetadata, AllURIMetadataNames) ->
    ...
    DeleteAttributeFunction =
        fun
            (?ACL_XATTR_NAME) ->
                ok = logical_file_manager:remove_acl(Auth, FileKey);
            (Name) ->
                ok = logical_file_manager:remove_xattr(Auth, FileKey, Name)
        end,
    ReplaceAttributeFunction =
        fun
            ({?ACL_XATTR_NAME, Value}) ->
                ...
                ok = logical_file_manager:set_acl(Auth, FileKey, ACL);
            ({Name, Value}) ->
...
%%--------------------------------------------------------------------
-spec prepare_metadata(rest_auth:auth(), logical_file_manager:file_key()) ->
    maps:map().
...
%%--------------------------------------------------------------------
-spec prepare_metadata(rest_auth:auth(), logical_file_manager:file_key(), binary()) ->
    maps:map().
prepare_metadata(Auth, FileKey, Prefix) ->
    {ok, Attrs} = logical_file_manager:stat(Auth, FileKey),
    prepare_metadata(Auth, FileKey, Prefix, Attrs).

%%--------------------------------------------------------------------
%% @doc Prepares cdmi user and storage system metadata with given prefix.
%%--------------------------------------------------------------------
-spec prepare_metadata(Auth :: rest_auth:auth(), FileKey :: logical_file_manager:file_key(),
...
%% @end
%%--------------------------------------------------------------------
-spec get_mimetype(rest_auth:auth(), logical_file_manager:file_key()) -> binary().
get_mimetype(Auth, FileKey) ->
    case logical_file_manager:get_mimetype(Auth, FileKey) of
        {ok, Value} ->
            Value;
        {error, ?ENOATTR} ->
...
%% @end
%%--------------------------------------------------------------------
-spec get_encoding(rest_auth:auth(), logical_file_manager:file_key()) -> binary().
get_encoding(Auth, FileKey) ->
    case logical_file_manager:get_transfer_encoding(Auth, FileKey) of
        {ok, Value} ->
            Value;
        {error, ?ENOATTR} ->
...
%% @end
%%--------------------------------------------------------------------
-spec get_cdmi_completion_status(rest_auth:auth(), logical_file_manager:file_key()) -> binary().
get_cdmi_completion_status(Auth, FileKey) ->
    case logical_file_manager:get_cdmi_completion_status(Auth, FileKey) of
        {ok, Value} ->
            Value;
        {error, ?ENOATTR} ->
...
%%--------------------------------------------------------------------
-spec update_mimetype(rest_auth:auth(), logical_file_manager:file_key(), binary()) -> ok | no_return().
update_mimetype(_Auth, _FileKey, undefined) -> ok;
update_mimetype(Auth, FileKey, Mimetype) ->
    ok = logical_file_manager:set_mimetype(Auth, FileKey, Mimetype).

%%--------------------------------------------------------------------
%% @doc Updates valuetransferencoding associated with file
%%--------------------------------------------------------------------
-spec update_encoding(rest_auth:auth(), logical_file_manager:file_key(), binary() | undefined) -> ok | no_return().
update_encoding(_Auth, _FileKey, undefined) -> ok;
update_encoding(Auth, FileKey, Encoding) ->
    ok = logical_file_manager:set_transfer_encoding(Auth, FileKey, Encoding).

%%--------------------------------------------------------------------
%% @doc Updates completion status associated with file
%%--------------------------------------------------------------------
-spec update_cdmi_completion_status(rest_auth:auth(), logical_file_manager:file_key(), binary()) ->
    ok | no_return().
...
update_cdmi_completion_status(Auth, FileKey, CompletionStatus)
    ...
    ok = logical_file_manager:set_cdmi_completion_status(Auth, FileKey, CompletionStatus).

%%--------------------------------------------------------------------
%% @doc Updates completion status associated with file
%% according to X-CDMI-Partial flag
%%--------------------------------------------------------------------
-spec set_cdmi_completion_status_according_to_partial_flag(rest_auth:auth(), logical_file_manager:file_key(), binary()) ->
    ok | no_return().
...
%%--------------------------------------------------------------------
-spec prepare_cdmi_metadata(MetadataNames :: [binary()], logical_file_manager:file_key(),
    ...
    case str_utils:binary_starts_with(Name, Prefix) of
        true ->
            case Name of
                ...
                ?ACL_XATTR_NAME ->
                    case logical_file_manager:get_acl(Auth, FileKey) of
                        {ok, Acl} ->
                            (prepare_cdmi_metadata(Rest, FileKey, Auth, Attrs, Prefix))#{
