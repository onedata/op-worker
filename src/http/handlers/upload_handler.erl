...
-define(MAX_WAIT_FOR_FILE_HANDLE, 2000).
...
handle_http_upload(Req) ->
    ...
    InitSession =
        try
            ...
            % @todo debug
            ?dump(g_session:get_value(?UPLOAD_MAP))
        catch _:_ ->
                ...
                {file, _FieldName, _Filename, _CType, _CTransferEncoding} ->
                    Print = lists:foldl(
                        fun({FieldName, FieldValue}, Acc) ->
                            Acc ++ str_utils:format("   ~s: ~s~n", [FieldName, FieldValue])
                        end, str_utils:format("stream_file: {~n", []), Params),
                    io:format("~s}~n~n", [Print]),
...
stream_file(Req, FileHandle, Offset) ->
    case cowboy_req:part_body(Req) of
        {ok, Body, Req2} ->
            ...
            % @todo register_chunk? or send a message from client that uuid has finished?
            Req2;
        {more, Body, Req2} ->
...
wait_for_file_new_file_id(Identifier, Timeout) ->
    case upload_map_lookup(Identifier, undefined) of
        undefined ->
            ?dump({wait_for_file_handle, Timeout}),
...
%%register_chunk(FileHandle) ->
%%    Result = ets:update_counter(upload_ets, {counter, FileHandle}, 1),
%%    [{_, TotalChunks}] = ets:lookup(?UPLOAD_ETS, {total_chunks, FileHandle}),
%%    ?dump({Result, TotalChunks}),
%%    case Result of
%%        TotalChunks ->
%%            % Upload has finished, clean the ETS
%%            % Get the identifier based on FileHandle
%%            [{{identifier, FileHandle}, Identifier}] =
%%                ets:lookup(?UPLOAD_ETS, {identifier, FileHandle}),
%%            % @ TODO FILE HANDLE SIE ZMIENIA!
%%            ets:delete_object(?UPLOAD_ETS, {identifier, FileHandle}),
%%            ets:delete_object(?UPLOAD_ETS, {total_chunks, FileHandle}),
%%            ets:delete_object(?UPLOAD_ETS, {counter, FileHandle}),
%%            ets:delete_object(?UPLOAD_ETS, {file_handle, Identifier}),
%%            ?dump(ets_clean),
%%            ets:foldl(fun(El, _) -> ?dump(El) end, [], ?UPLOAD_ETS),
%%            ok;
%%        _ ->
%%            ok
%%    end,
