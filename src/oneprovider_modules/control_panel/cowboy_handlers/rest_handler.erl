...
get_resource(Req, #state{version = Version, handler_module = Mod, resource_id = Id} = State) ->
    ...
    {Resp, NewReq} = case Answer of
                         ...
                         error ->
                             {ok, Req3} = opn_cowboy_bridge:apply(cowboy_req, reply, [500, Req2]),
                             {halt, Req3};
                         {error, ErrorDesc} ->
                             Req3 = cowboy_req:set_resp_body(ErrorDesc, Req2),
                             {ok, Req4} = opn_cowboy_bridge:apply(cowboy_req, reply, [500, Req3]),
                             {halt, Req4}
                     end,
    {Resp, NewReq, State}.
...
handle_json_data(Req, #state{handler_module = Mod, version = Version, resource_id = Id} = State) ->
    {ok, Binary, Req2} = opn_cowboy_bridge:apply(cowboy_req, body, [Req]),
    Data = case Binary of
               <<"">> ->
                   <<"">>;
