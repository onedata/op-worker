...
%% Two fields are required when using oneprovider_upload:
...
% Must be the same as constants in oneprovider_upload.js
...
render_element(Record) ->
    TargetDir = Record#oneprovider_upload.target_dir,
    SubscriberPid = Record#oneprovider_upload.subscriber_pid,
    ...
    SubmitJS = <<"function (e){ oneprovider_send_pending_files($('#", ?FORM_ID, "').get(0), $('#", ?SELECT_BUTTON_ID, "').get(0)); }">>,
    ...
    UploadJS = <<"oneprovider_attach_upload_handle_dragdrop($('#", ?FORM_ID, "').get(0), $('#", ?SELECT_BUTTON_ID, "').get(0));">>,
