                ...
                #table{
                    ...
                    body = #tbody{
                        body = #tr{
                            ...
                            cells = lists:map(fun({TableId, Body}) ->
                                #th{
                                    style = <<"width: 50%;">>,
                                    body = #table{
                                ...
                                {<<"client_tokens">>, tokens_table_collapsed(<<"client_tokens">>, ?CLIENT_TOKENS_TABLE_NAME, [])},
                                {<<"provider_tokens">>, tokens_table_collapsed(<<"provider_tokens">>, ?PROVIDER_TOKENS_TABLE_NAME, [])}
                            ])
                        }
                    }
                }
...
tokens_table_collapsed(TableId, TableName, TokensDetails) ->
    NavigationBody = opn_gui_utils:expand_button(<<"Expand All">>, {message, {expand_tokens_table, TableId, TableName}}),
...
tokens_table_expanded(TableId, TableName, TokensDetails) ->
    NavigationBody = opn_gui_utils:collapse_button(<<"Collapse All">>, {message, {collapse_tokens_table, TableId, TableName}}),
...
token_row_collapsed(TableName, RowId, #token_details{access_id = AccessId, client_name = ClientName} = TokenDetails) ->
    [
        ...
        #td{
            style = ?NAVIGATION_COLUMN_STYLE,
            body = opn_gui_utils:expand_button({message, {expand_token_row, TableName, RowId, TokenDetails}})
        }
    ].
...
token_row_expanded(TableName, RowId, #token_details{access_id = AccessId} = TokenDetails) ->
    [
        ...
        #td{
            style = ?NAVIGATION_COLUMN_STYLE,
            body = opn_gui_utils:collapse_button({message, {collapse_token_row, TableName, RowId, TokenDetails}})
        }
    ].
        ...
        try
            receive
                ...
                get_authorization_code ->
                    case gr_openid:get_client_authorization_code({user, opn_gui_utils:get_access_token()}) of
                ...
                {change_client_name, TableName, RowId, #token_details{access_id = AccessId} = TokenDetails, NewClientName} ->
                    ...
                    NextState =
                        case ChangeClientNameFun({user, opn_gui_utils:get_access_token()}, AccessId, [{<<"clientName">>, NewClientName}]) of
                            ok ->
...
event(init) ->
    try
        ...
        {ok, ClientTokens} = gr_openid:get_client_tokens({user, opn_gui_utils:get_access_token()}),
        {NewClientTokens, _} = TokensFun(<<"client_token_">>, ClientTokens),
        {ok, ProviderTokens} = gr_openid:get_provider_tokens({user, opn_gui_utils:get_access_token()}),
