...
validate_login() ->
    try
        ...
        try
            ...
            opn_gui_utils:set_global_user_id(gui_str:binary_to_unicode_list(GRUID)),
            opn_gui_utils:set_access_token(AccessToken),
            opn_gui_utils:set_user_fullname(user_logic:get_name(UserDoc)),
            opn_gui_utils:set_user_role(user_logic:get_role(UserDoc)),
            opn_gui_utils:set_logout_token(opn_gui_utils:gen_logout_token()),
            ?debug("User ~p logged in", [Login]),

            #db_document{uuid = UserId} = UserDoc,
...
refresh_access(UserId) ->
    {ok, #db_document{record = User} = UserDoc} = user_logic:get_user({uuid, UserId}),
...
get_user_login(GRUID) ->
    case user_logic:get_user({global_id, GRUID}) of
        {ok, #db_document{record = #user{login = Login}}} -> Login;
