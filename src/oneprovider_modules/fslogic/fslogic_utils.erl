...
%% ====================================================================
file_to_space_info(#db_document{record = #file{} = File}) ->
...
get_sh_and_id(FuseID, Storage, File_id, SpaceId, ForceClusterProxy) ->
    ...
    case SHName =:= "ClusterProxy" of
        true ->
            {SHI#storage_helper_info{init_args = [binary_to_list(opn_utils:ensure_binary(SpaceId))]},
...
%% db_documents that describe children.
%% @end
...
create_children_list([File | Rest], Ans) ->
    FileDesc = File#db_document.record,
