...
get_new_file_id(#space_info{} = SpaceInfo, File, UserDoc, SHInfo, ProtocolVersion) ->
  {Root1, {CountStatus, FilesCount}} =
    case {string:tokens(File, "/"), UserDoc} of
      ...
      {_, #db_document{uuid = ?CLUSTER_USER_ID}} ->
        {"/", fslogic_utils:get_files_number(user, UserDoc#db_document.uuid, ProtocolVersion)}
    end,
  File_id_beg =
    case CountStatus of
  ...
  case FuseGroup of
    default ->
      case dao_lib:apply(dao_cluster, get_fuse_session, [FuseID], 1) of
        {ok, #db_document{record = #fuse_session{client_storage_info = ClientStorageInfo}}} ->
...
get_fuse_group(FuseID) ->
  case FuseID of
    ...
    _ ->
      ...
      case DAOStatus of
        ok ->
          Record = DAOAns#db_document.record,
