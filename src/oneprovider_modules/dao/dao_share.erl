...
%% or #db_document{} that wraps #share_desc{} if you want to update descriptor in DB. <br/>
...
save_file_share(#share_desc{} = Share) ->
  save_file_share(#db_document{record = Share});
save_file_share(#db_document{record = #share_desc{}} = FdDoc) ->
...
remove_file_share(Key) ->
  ...
  case Docs of
    D when is_record(D, db_document) ->
      remove_file_share({uuid, D#db_document.uuid});
    _ ->
      Rem = fun(Doc, TmpAns) ->
        RemAns = remove_file_share({uuid, Doc#db_document.uuid}),
        case RemAns of
          ok -> TmpAns;
          _ -> error
        end
...
%% Non-error return value is always {ok, #db_document{record = #share_desc}.
%% See {@link dao_records:save_record/1} and {@link dao_records:get_record/1} for more details about #db_document{} wrapper.<br/>
%% Should not be used directly, use {@link dao_worker:handle/2} instead (See {@link dao_worker:handle/2} for more details).
%% @end
