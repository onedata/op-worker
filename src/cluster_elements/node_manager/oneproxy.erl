...
get_session(OneProxyNameOrPid, SessionId) ->
    exec(OneProxyNameOrPid, <<"get_session">>, [<<(vcn_utils:ensure_binary(SessionId))/binary>>]).
...
start(ListenerPort, ForwardPort, CertFile, VerifyType) ->
    ...
    Port = open_port({spawn_executable, ExecPath}, [
        ...
        {args, [integer_to_list(ListenerPort), "127.0.0.1", integer_to_list(ForwardPort), vcn_utils:ensure_list(CertFile),
            vcn_utils:ensure_list(VerifyType), filename:join(CADir, ?DER_CERTS_DIR)]}
    ]),
    try
    ...
    NewState =
        receive
            ...
            {{Pid, Id}, {command, CMD, Args}} ->
                ...
                PidSize = size(BinPid),
                ArgsBin = vcn_utils:binary_join(Args, <<" ">>),
