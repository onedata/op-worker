...
lfm_create_and_read_symlink(Config) ->
    ...
    ?assertMatch({ok, [LinkAttrs2], _}, lfm_proxy:get_children_attrs(W, SessId, ?FILE_REF(DirGuid), #{offset => 0, limit => 10, optimize_continuous_listing => false})),
    ...
    ?assertMatch({ok, [], _}, lfm_proxy:get_children_attrs(W, SessId, ?FILE_REF(DirGuid), #{offset => 0, limit => 10, optimize_continuous_listing => false})),
