...
create_file_import_race_test(Config) ->
    ...
    ok = test_utils:mock_expect(Workers, storage_sync_engine, import_file_unsafe, fun(StorageFileCtx, Info) ->
        storage_sync_test_base:block_syncing_process(TestPid),
    ...
    SyncingProcess = storage_sync_test_base:await_syncing_process(),
    ...
    storage_sync_test_base:release_syncing_process(SyncingProcess),
    ...
    ProviderId1 = storage_sync_test_base:provider_id(W1),
...
close_file_import_race_test(Config) ->
    ...
    ok = test_utils:mock_expect(Workers, link_utils, try_to_resolve_child_deletion_link, fun(FileName, ParentCtx) ->
        case FileName =:= ?TEST_FILE1 of
             true -> storage_sync_test_base:block_syncing_process(TestPid);
            _ -> ok
        end,
    ...
    ContainerStorageSpacePath = storage_sync_test_base:storage_path(RDWRStorageMountPoint, ?SPACE_ID, <<"">>, MountInRoot),
    storage_sync_test_base:bump_mtime(W1, ContainerStorageSpacePath),
...
delete_file_reimport_race_test(Config) ->
    ...
    ok = test_utils:mock_expect(W1, storage_sync_engine, check_location_and_maybe_sync, fun(StorageFileCtx, FileCtx, Info) ->
        ...
        case Guid =:= FileGuid of
            true -> storage_sync_test_base:block_syncing_process(TestProcess);
            false -> ok
        end,
...
sync_should_not_reimport_directory_that_was_not_successfully_deleted_from_storage(Config) ->
    ...
    StorageTestDirPath = storage_sync_test_base:storage_path(?SPACE_ID, TestDir, MountSpaceInRoot),
...
%%    storage_sync_test_base:change_time(StorageSpaceDirPath, StMtime + 1),
    ...
    storage_sync_test_base:enable_update(Config, ?SPACE_ID, RDWRStorage),
...
create_delete_race_test(Config) ->
    storage_sync_test_base:create_delete_race_test(Config, false).
