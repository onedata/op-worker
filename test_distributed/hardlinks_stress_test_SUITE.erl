...
many_files_creation_tree_test(Config) ->
    ?PERFORMANCE(Config, [
        {parameters, [
            ...
            [{name, dir_level}, {value, 4}, {description, "Level of last test directory"}],
            [{name, dirs_per_parent}, {value, 4}, {description, "Child directories in single dir"}],
            [{name, files_per_dir}, {value, 4}, {description, "Number of files in single directory"}]
        ]},
...
end_per_testcase(stress_test = Case, Config) ->
    stress_test_traverse_pool:stop_pool(Config),
