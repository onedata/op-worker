...
%% @end
%% ===================================================================
%% @doc: This module tests the functionality of dns_udp_sup,
...
-export([all/0]).
-export([dns_udp_sup_env_test/1, dns_udp_sup_returns_right_child_specs_test/1, dns_udp_handler_responds_to_dns_queries/1, dns_ranch_tcp_handler_responds_to_dns_queries/1]).

-include("env_setter.hrl").
-include("registered_names.hrl").

all() -> [dns_udp_sup_env_test, dns_udp_sup_returns_right_child_specs_test, dns_ranch_tcp_handler_responds_to_dns_queries, dns_udp_handler_responds_to_dns_queries].
...
dns_udp_sup_env_test(_Config) ->
	try
		...
		assert_all_deps_are_met(?APP_Name, dns_udp_sup)
	after
		env_setter:stop_test()
	end.

%% Checks if dns_udp_sup returns expected child specification
dns_udp_sup_returns_right_child_specs_test(_Config) ->
	try
		?INIT_DIST_TEST,
		env_setter:start_test(),

		DNS_RESPONSE_TTL = 2,
		Dispatcher_Timeout = 3,
		Port = 1308,
		application:set_env(?APP_Name, dns_port, Port),
		application:set_env(?APP_Name, dns_response_ttl, DNS_RESPONSE_TTL),
		application:set_env(?APP_Name, dispatcher_timeout, Dispatcher_Timeout),

		ExpectedChilds = [{dns_udp_acceptor, {dns_udp_handler, start_link, [Port, DNS_RESPONSE_TTL, Dispatcher_Timeout]},
			permanent, 5000, worker, [dns_udp_handler]}],

		{ok, {{one_for_one, 10, 5}, ChildSpecs}} = dns_udp_sup:init([]),

		ExpectedChilds = ChildSpecs
	after
		env_setter:stop_test()
	end.
...
dns_udp_handler_responds_to_dns_queries(_Config) ->
	...
	UDP_Port = 1400,
	Address = "localhost",
	SupportedDomain = "dns_worker.xxxx.xxx.xxx",
	...
	try
		...
		%% dns udp handler should work, but dns_worker should not
		FirstMessagingTest = fun () ->
			...
			?SERVFAIL = Header#dns_header.rcode
		end,
