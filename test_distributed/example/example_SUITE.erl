...
init_per_testcase(distributed_test, Config) ->
  ...
  test_node_starter:start_app_on_nodes(?APP_Name, ?ONEDATA_DEPS, Nodes, [[{node_type, ccm_test}, {dispatcher_port, 5055}, {ccm_nodes, [Node1]}, {dns_port, 1308}, {control_panel_port, 2308}, {control_panel_redirect_port, 1354}, {rest_port, 3308}, {heart_beat, 1},{nif_prefix, './'},{ca_dir, './cacerts/'}],
...
init_per_testcase(local_test, Config) ->
  ?INIT_CODE_PATH,?CLEAN_TEST_DIRS,
  test_node_starter:start_deps(?ONEDATA_DEPS),
  test_node_starter:start_app_on_nodes(?APP_Name, ?ONEDATA_DEPS,[node()],[[{node_type, ccm_test}, {dispatcher_port, 7777}, {ccm_nodes, [node()]}, {dns_port, 1312},{nif_prefix, './'},{ca_dir, './cacerts/'}]]),
  Config.
...
end_per_testcase(local_test, _Config) ->
  test_node_starter:stop_app_on_nodes(?APP_Name, ?ONEDATA_DEPS, [node()]).
