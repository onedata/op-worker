...
mv_and_sync_file(UserSelector, FileSelector, DstPath) ->
    ...
    lists:foreach(fun(Provider) ->
        ...
        ?assertEqual({ok, DstPath}, opt_file_tree:get_path(Node, UserSessId, FileGuid), ?ATTEMPTS)
    end, RestProviders).
...
prepare_symlink_value(Node, SessId, FileGuid) ->
    ...
    {ok, CanonicalPath} = opt_file_tree:get_path(Node, SessId, FileGuid),
...
create_hardlink(Node, SessId, ParentGuid, FileName, TargetGuid) ->
    {ok, ParentPath} = opt_file_tree:get_path(Node, SessId, ParentGuid),
