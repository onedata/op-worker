...
rtransfer_restart_test(Config) ->
    ...
    restart_test_base(Config, RestartFun).
...
restart_test_base(Config, RestartFun) ->
    ...
    Files = lists:map(fun(_) ->
        ...
        {ok, FileGuid} = ?assertMatch({ok, _}, rpc:call(WorkerP1, lfm, create, [SessIdP1, SpaceGuid, File, 8#755], 5000)),
        {ok, Handle} = ?assertMatch({ok, _}, rpc:call(WorkerP1, lfm, open, [SessIdP1, {guid, FileGuid}, rdwr], 5000)),
        {ok, NewHandle, _} = ?assertMatch({ok, _, _}, rpc:call(WorkerP1, lfm, write,  [Handle, 0, ?FILE_DATA], 5000)),
        ?assertEqual(ok, rpc:call(WorkerP1, lfm, fsync, [NewHandle], 35000)),
        ?assertEqual(ok, rpc:call(WorkerP1, lfm, release, [NewHandle], 5000)),
        FileGuid
