...
-export([basic_test/1, hardlinks_test/1, multiprovider_hardlinks_test/1,
    ...
    restart_test/1, parallel_write_test/4, local_opened_file_deletion_closing_race_base/1]).
...
local_opened_file_deletion_closing_race_base(Config) ->
    enable(Config),
    ...
    ?assertMatch({ok, #{?VIRTUAL_SIZE := 8, ?LOGICAL_SIZE := 8}}, rpc:call(Node, dir_size_stats, get_stats, [SpaceGuid]), ?ATTEMPTS),
    DeleteFun = fun() ->
        ok = lfm_proxy:unlink(Node, SessId, #file_ref{guid = FileGuid})
    end,
    CloseFun = fun() ->
        ok = lfm_proxy:close(Node, Handle)
    end,
    lists_utils:pforeach(fun(Fun) -> Fun() end, [DeleteFun, CloseFun]),
