...
local_opened_file_deletion_closing_race_base(Config, FilesNum) ->
    ...
    ?assertMatch({ok, #{?VIRTUAL_SIZE := 0, ?LOGICAL_SIZE := 0}}, rpc:call(Node, dir_size_stats, get_stats, [SpaceDirGuid]), ?ATTEMPTS).
...
check_space_dir_values_map_and_time_series_collection(
    ...
    ?assertMatch(?ERR_DIR_STATS_DISABLED_FOR_SPACE, rpc:call(Worker, dir_size_stats, get_stats, [SpaceDirGuid]));
