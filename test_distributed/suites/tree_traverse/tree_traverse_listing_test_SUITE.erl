...
qos_traverse_listing_error_base() ->
    #object{guid = RootDirGuid} = onenv_file_test_utils:create_file_tree(
...
archivisation_traverse_listing_error_base(ErrorType) ->
    onenv_file_test_utils:create_file_tree(
        ...
        fslogic_file_id:spaceid_to_space_dir_guid(oct_background:get_space_id(space1)),
        oct_background:get_provider_id(krakow),
        #dir_spec{dataset = #dataset_spec{archives = 1}}
    ),
    KrakowNode = oct_background:get_random_provider_node(krakow),
...
archive_recall_traverse_listing_error_base() ->
    #object{guid = RootDirGuid, dataset = #dataset_object{archives = [#archive_object{id = ArchiveId}]}} = onenv_file_test_utils:create_file_tree(
        oct_background:get_user_id(user1),
        fslogic_file_id:spaceid_to_space_dir_guid(oct_background:get_space_id(space1)),
        oct_background:get_provider_id(krakow),
        #dir_spec{dataset = #dataset_spec{archives = 1}}
    ...
    % traverse should retry listing until it succeeds
    timer:sleep(timer:seconds(8)),
    ?assertEqual({ok, 1}, get_number_of_ongoing_traverses(KrakowNode, <<"archive_recall_traverse">>), ?ATTEMPTS),
    test_utils:mock_assert_num_calls_sum(KrakowNode, archive_recall_traverse, task_finished, 2, 0, ?ATTEMPTS),
    unmock_listing(),
    test_utils:mock_assert_num_calls_sum(KrakowNode, archive_recall_traverse, task_finished, 2, 1, ?ATTEMPTS).
...
replica_eviction_traverse_listing_error_base(ErrorType) ->
    ...
    ok = opw_test_rpc:call(krakow, replica_eviction_traverse, start, [<<"dummy_provider_id">>, #document{value = #transfer{file_uuid = FileUuid, space_id = SpaceId}}]),
    
    case ErrorType of
        unexpected ->
            % traverse should retry listing until it succeeds
            timer:sleep(timer:seconds(8)),
            ?assertEqual({ok, 1}, get_number_of_ongoing_traverses(KrakowNode, replica_eviction_traverse:pool_name()), ?ATTEMPTS),
            test_utils:mock_assert_num_calls_sum(KrakowNode, transfer_file_tree_traverse, task_finished, 2, 0, ?ATTEMPTS),
            unmock_listing(),
...
replication_traverse_listing_error_base(ErrorType) ->
    ...
    ok = opw_test_rpc:call(krakow, replication_traverse, start, [#document{value = #transfer{file_uuid = FileUuid, space_id = SpaceId}}]),
    
    case ErrorType of
        unexpected ->
            % traverse should retry listing until it succeeds
            timer:sleep(timer:seconds(8)),
            ?assertEqual({ok, 1}, get_number_of_ongoing_traverses(KrakowNode, replication_traverse:pool_name()), ?ATTEMPTS),
            test_utils:mock_assert_num_calls_sum(KrakowNode, transfer_file_tree_traverse, task_finished, 2, 0, ?ATTEMPTS),
            unmock_listing(),
            test_utils:mock_assert_num_calls_sum(KrakowNode, transfer_file_tree_traverse, task_finished, 2, 1, ?ATTEMPTS);
        known ->
...
bulk_download_traverse_listing_error_base(ErrorType) ->
    ...
    {ok, _} = opw_test_rpc:call(krakow, bulk_download_traverse, start, [datastore_key:new(), UserCtx, RootDirGuid, preserve, <<>>]),
    
    case ErrorType of
        unexpected ->
            % traverse should retry listing until it succeeds
            timer:sleep(timer:seconds(8)),
            ?assertEqual({ok, 1}, get_number_of_ongoing_traverses(KrakowNode, <<"bulk_download_traverse">>), ?ATTEMPTS),
            test_utils:mock_assert_num_calls_sum(KrakowNode, bulk_download_traverse, task_finished, 2, 0, ?ATTEMPTS),
            unmock_listing(),
...
tree_deletion_traverse_listing_error_base(ErrorType) ->
    ...
    case ErrorType of
        ...
        known ->
            ...
            timer:sleep(timer:seconds(8)),
            ?assertEqual({ok, 1}, get_number_of_ongoing_traverses(KrakowNode, <<"tree_deletion_traverse">>), ?ATTEMPTS),
            test_utils:mock_assert_num_calls_sum(KrakowNode, tree_deletion_traverse, task_finished, 2, 0, ?ATTEMPTS),
            unmock_listing(),
            test_utils:mock_assert_num_calls_sum(KrakowNode, tree_deletion_traverse, task_finished, 2, 1, ?ATTEMPTS)
    end.
...
dir_stats_collections_initialization_traverse_listing_error_base(ErrorType) ->
    ...
    case ErrorType of
        ...
        known ->
            % traverse should retry listing until it succeeds
            timer:sleep(timer:seconds(8)),
            test_utils:mock_assert_num_calls_sum(KrakowNode, dir_stats_collections_initialization_traverse, task_finished, 2, 0, ?ATTEMPTS),
            unmock_listing(),
            test_utils:mock_assert_num_calls_sum(KrakowNode, dir_stats_collections_initialization_traverse, task_finished, 2, 1, ?ATTEMPTS)
    end.
