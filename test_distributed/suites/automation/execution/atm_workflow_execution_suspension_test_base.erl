...
-define(TASK_SELECTOR, {{1, 1}, <<"pb">>, <<"t1">>}).
            ...
            handle_exception = #atm_step_mock_spec{
                after_step_exp_state_diff = fun(#atm_mock_call_ctx{workflow_execution_exp_state = ExpState0}) ->
                    ExpState1 = atm_workflow_execution_exp_state_builder:expect_task_interrupted(
                        get_task_id(ExpState0), ExpState0
                    ),
                    ExpState2 = atm_workflow_execution_exp_state_builder:expect_lane_run_interrupted({1, 1}, ExpState1),
                    {true, atm_workflow_execution_exp_state_builder:expect_workflow_execution_interrupted(ExpState2)}
                end
            }
        }]
    }).
...
%% @private
get_task_id(ExpState) ->
    atm_workflow_execution_exp_state_builder:get_task_id(?TASK_SELECTOR, ExpState).
