...
create_test(_Config) ->
    ...
    ?assertThrow(
        ...
        ?erpc(atm_store_api:create(AtmWorkflowExecutionAuth, ?LOGGER_DEBUG_LEVEL, [], AtmStoreSchema))
    ),

    {ok, #document{key = AtmStoreId}} = ?assertMatch(
        {ok, #document{value = #atm_store{initial_content = undefined, frozen = false}}},
        ?rpc(atm_store_api:create(AtmWorkflowExecutionAuth, ?LOGGER_DEBUG_LEVEL, undefined, AtmStoreSchema))
    ),
