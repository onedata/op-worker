...
-define(TEST_PROTECTION_FLAGS, ?set_flags(?no_flags_mask, ?METADATA_PROTECTION)).
...
establish_dataset_attached_to_space_dir(_Config) ->
    ...
    {ok, DatasetId} = ?assertMatch({ok, _}, lfm_proxy:establish_dataset(P1Node, UserSessIdP1, {guid, SpaceGuid}, ?TEST_PROTECTION_FLAGS)),
    ?assertAttachedDataset(P1Node, UserSessIdP1, DatasetId, SpaceGuid, undefined, ?TEST_PROTECTION_FLAGS),
    ?assertDatasetMembership(P1Node, UserSessIdP1, SpaceGuid, ?DIRECT_DATASET_MEMBERSHIP, ?TEST_PROTECTION_FLAGS),
    ?assertFileEffDatasetSummary(P1Node, UserSessIdP1, SpaceGuid, DatasetId, [], ?TEST_PROTECTION_FLAGS).
...
establish_dataset_attached_to_dir(_Config) ->
    ...
    {ok, DatasetId} = ?assertMatch({ok, _}, lfm_proxy:establish_dataset(P1Node, UserSessIdP1, {guid, Guid}, ?TEST_PROTECTION_FLAGS)),
    ?assertAttachedDataset(P1Node, UserSessIdP1, DatasetId, Guid, undefined, ?TEST_PROTECTION_FLAGS),
    ?assertDatasetMembership(P1Node, UserSessIdP1, Guid, ?DIRECT_DATASET_MEMBERSHIP, ?TEST_PROTECTION_FLAGS),
    ?assertFileEffDatasetSummary(P1Node, UserSessIdP1, Guid, DatasetId, [], ?TEST_PROTECTION_FLAGS).
...
detach_and_reattach_dataset(_Config) ->
    ...
    ?assertDetachedDataset(P1Node, UserSessIdP1, DatasetId, Guid, undefined, Path, ?DIRECTORY_TYPE, ?TEST_PROTECTION_FLAGS),
...
establish_dataset_on_not_existing_file_should_fail(_Config) ->
    ...
    {ok, Guid} = lfm_proxy:create(P1Node, UserSessIdP1, SpaceGuid, FileName, ?DEFAULT_DIR_PERMS),
    ok = lfm_proxy:unlink(P1Node, UserSessIdP1, {guid, Guid}),
    ?assertMatch({error, ?ENOENT}, lfm_proxy:establish_dataset(P1Node, UserSessIdP1, {guid, Guid}, ?TEST_PROTECTION_FLAGS)).
...
establish_2nd_dataset_on_file_should_fail(_Config) ->
    ...
    ?assertDatasetMembership(P1Node, UserSessIdP1, Guid, ?DIRECT_DATASET_MEMBERSHIP, ?TEST_PROTECTION_FLAGS),
    ?assertFileEffDatasetSummary(P1Node, UserSessIdP1, Guid, DatasetId, [], ?TEST_PROTECTION_FLAGS),
    ?assertMatch({error, ?EEXIST}, lfm_proxy:establish_dataset(P1Node, UserSessIdP1, {guid, Guid}, ?TEST_PROTECTION_FLAGS)).
...
establish_nested_datasets_structure(_Config) ->
    ...
    {ok, SpaceDatasetId} = lfm_proxy:establish_dataset(P1Node, UserSessIdP1, {guid, SpaceGuid}, ?TEST_PROTECTION_FLAGS),
    ...
    GuidsAndDatasetsReversed = lists:foldl(fun(N, AccIn = [{ParentGuid, _, _} | _]) ->
        ...
        {ok, DatasetId} = lfm_proxy:establish_dataset(P1Node, UserSessIdP1, {guid, Guid}, ?TEST_PROTECTION_FLAGS),
    ...
    lists:foldl(fun({ChildGuid, _ChildName, ChildDatasetId}, {Guid, DatasetId, ExpParentDatasetIds}) ->
        ...
        ?assertAttachedDataset(P1Node, UserSessIdP1, DatasetId, Guid, ExpParentDatasetId, ?TEST_PROTECTION_FLAGS),
        ?assertDatasetMembership(P1Node, UserSessIdP1, Guid, ?DIRECT_DATASET_MEMBERSHIP, ?TEST_PROTECTION_FLAGS),
        ?assertFileEffDatasetSummary(P1Node, UserSessIdP1, Guid, DatasetId, ExpParentDatasetIds, ?TEST_PROTECTION_FLAGS),
...
establish_nested_datasets_structure_end_detach_all(_Config) ->
    ...
    lists:foldl(fun({ChildGuid, _ChildName, ChildDatasetId}, {Guid, DatasetId, ExpParentDatasetId}) ->
        {ok, Path} = lfm_proxy:get_file_path(P1Node, UserSessIdP1, Guid),
        ?assertDetachedDataset(P1Node, UserSessIdP1, DatasetId, Guid, ExpParentDatasetId, Path, ?DIRECTORY_TYPE, ?TEST_PROTECTION_FLAGS),
        ?assertDatasetMembership(P1Node, UserSessIdP1, Guid, ?NONE_DATASET_MEMBERSHIP, ?no_flags_mask),
        ?assertFileEffDatasetSummary(P1Node, UserSessIdP1, Guid, DatasetId, [], ?no_flags_mask),
...
all_files_in_dataset_should_have_ancestor_dataset_membership(_Config) ->
    ...
    ?assertAttachedDataset(P1Node, UserSessIdP1, SpaceDatasetId, SpaceGuid, undefined, ?TEST_PROTECTION_FLAGS),
    ?assertMatch({ok, [], true},
        lfm_proxy:list_nested_datasets(P1Node, UserSessIdP1, SpaceDatasetId, #{offset => 0, limit => 100})),

    lists:foreach(fun(Guid) ->
        ?assertDatasetMembership(P1Node, UserSessIdP1, Guid, ?ANCESTOR_DATASET_MEMBERSHIP, ?TEST_PROTECTION_FLAGS),
        ?assertFileEffDatasetSummary(P1Node, UserSessIdP1, Guid, undefined, [SpaceDatasetId], ?TEST_PROTECTION_FLAGS)
...
rename_file_should_not_rename_detached_dataset(_Config) ->
    ...
    ?assertDetachedDataset(P1Node, UserSessIdP1, DatasetId, Guid, undefined, SourcePatch, ?DIRECTORY_TYPE, ?TEST_PROTECTION_FLAGS),
    ?assertDatasetMembership(P1Node, UserSessIdP1, Guid, ?NONE_DATASET_MEMBERSHIP, ?no_flags_mask),
    ?assertFileEffDatasetSummary(P1Node, UserSessIdP1, Guid, DatasetId, [], ?no_flags_mask).
