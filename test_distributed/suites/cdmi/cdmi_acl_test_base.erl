...
write_acl_metadata_test(Config) ->
    ...
    {ok, ?HTTP_201_CREATED, _Headers1, _Response1} = cdmi_test_utils:do_request(
        ?CDMI_WORKERS(Config), DirPath, put, RequestHeaders1, RawRequestBody1
    ),
    ...
    {ok, ?HTTP_204_NO_CONTENT, _Headers2, _Response2} = cdmi_test_utils:do_request(
        ?CDMI_WORKERS(Config), FilePath ++ "?metadata:cdmi_acl", put, RequestHeaders2, RawRequestBody2
    ),

    GetFileMetadataFun = fun() ->
        {ok, ?HTTP_200_OK, _Headers3, Response3} =
            cdmi_test_utils:do_request(
                ?CDMI_WORKERS(Config), FilePath ++ "?metadata", get, RequestHeaders2, []
        ),
        json_utils:decode(Response3)
    end,

    ?assertEqual(6, maps:size(maps:get(<<"metadata">>,  GetFileMetadataFun())), ?CDMI_ATTEMPTS),
    ...
    {ok, _Code4, _Headers4, Response4} = ?assertMatch(
        {ok, ?HTTP_200_OK, _, _},
        cdmi_test_utils:do_request(
        ?CDMI_WORKERS(Config), FilePath, get, [cdmi_test_utils:user_2_token_header()], []
    )),
    ...
    ?assertMatch(
        ...
        cdmi_test_utils:do_request(
            ?CDMI_WORKERS(Config), filename:join([DirPath, "?metadata:cdmi_acl"]), put, RequestHeaders3, RawRequestBody3
        ),
        ?CDMI_ATTEMPTS
    ),
    ...
    {ok, ?HTTP_201_CREATED, _, _} = cdmi_test_utils:do_request(
        ?CDMI_WORKERS(Config), DirPath2, put, RequestHeaders1, RawRequestBody1
    ),
    ?assertMatch({ok, ?HTTP_204_NO_CONTENT, _, _}, cdmi_test_utils:do_request(
        ?CDMI_WORKERS(Config), filename:join([DirPath2, "?metadata:cdmi_acl"]), put, RequestHeaders3, RawRequestBody4
    ), ?CDMI_ATTEMPTS),

    Fun2 = fun() ->
        {ok, Code7, _Headers7, Response7} = cdmi_test_utils:do_request(
            ?CDMI_WORKERS(Config), filename:join(DirPath, "some_file"), put, [cdmi_test_utils:user_2_token_header()], []
        ),
        {Code7, json_utils:decode(Response7)}
    end,
    ExpRestError = rest_test_utils:get_rest_error(?ERR_POSIX(?EACCES)),
    ?assertMatch(ExpRestError, Fun2(), ?CDMI_ATTEMPTS).
...
acl_read_file_test(Config) ->
    ...
    {ok, ?HTTP_204_NO_CONTENT, _, _} = cdmi_test_utils:do_request(
        ?CDMI_WORKERS(Config), FilePath, put, RequestHeaders, MetadataAclWrite
    ),
    {ok, Code1, _, Response1} = ?assertMatch(
        {ok, ?HTTP_400_BAD_REQUEST, _, _},
        cdmi_test_utils:do_request(?CDMI_WORKERS(Config), FilePath, get, RequestHeaders, []),
        ?CDMI_ATTEMPTS
    ),
    ...
    {ok, Code2, _, Response2} = ?assertMatch(
        {ok, ?HTTP_400_BAD_REQUEST, _, _},
        cdmi_test_utils:do_request(?CDMI_WORKERS(Config), FilePath, get, [cdmi_test_utils:user_2_token_header()], []),
        ?CDMI_ATTEMPTS
    ),
    ?assertMatch(EaccesError, {Code2, json_utils:decode(Response2)}),
    ?assertEqual({error, ?EACCES}, cdmi_test_utils:open_file(
        Config#cdmi_test_config.p2_selector, FilePath, read), ?CDMI_ATTEMPTS
    ),

    % set acl to 'read&write' and test cdmi/non-cdmi get request (should succeed)
    ?assertMatch({ok, ?HTTP_204_NO_CONTENT, _, _}, cdmi_test_utils:do_request(
            ?CDMI_WORKERS(Config), FilePath, put, RequestHeaders, MetadataAclReadWriteFull
    ), ?CDMI_ATTEMPTS),
...
acl_write_file_test(Config) ->
    ...
    ?assertEqual(<<"new_data">>, cdmi_test_utils:get_file_content(FilePath, Config), ?CDMI_ATTEMPTS),

    ?assertMatch({ok, _}, cdmi_test_utils:write_to_file(FilePath, <<"1">>, 8, Config), ?CDMI_ATTEMPTS),
    ?assertEqual(<<"new_data1">>, cdmi_test_utils:get_file_content(FilePath, Config), ?CDMI_ATTEMPTS),

    ?assertMatch({ok, ?HTTP_204_NO_CONTENT, _, _}, cdmi_test_utils:do_request(
        WorkerP1, FilePath, put, [cdmi_test_utils:user_2_token_header()], <<"new_data2">>
    ), ?CDMI_ATTEMPTS),

    ?assertEqual(<<"new_data2">>, cdmi_test_utils:get_file_content(FilePath, Config), ?CDMI_ATTEMPTS),
...
acl_delete_file_test(Config) ->
    ...
    ?assert(not cdmi_test_utils:object_exists(FilePath, Config), ?CDMI_ATTEMPTS).
...
acl_read_write_dir_test(Config) ->
    ...
    ?assert(cdmi_test_utils:object_exists(File1, Config), ?CDMI_ATTEMPTS),

    {ok, ?HTTP_201_CREATED, _, _} = cdmi_test_utils:do_request(
        WorkerP1, File2, put, RequestHeaders1, <<"{\"value\":\"val\"}">>
    ),
    ?assert(cdmi_test_utils:object_exists(File2, Config), ?CDMI_ATTEMPTS),
    ?assert(cdmi_test_utils:object_exists(File3, Config), ?CDMI_ATTEMPTS),
    ...
    {ok, ?HTTP_200_OK, _, _} = cdmi_test_utils:do_request(?CDMI_WORKERS(Config), Dirname, get, RequestHeaders2, []),
