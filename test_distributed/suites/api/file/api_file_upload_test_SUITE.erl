...
build_rest_create_file_under_path_prepare_args_fun(MemRef, ParentDirObjectId) ->
    fun(#api_test_ctx{data = Data0}) ->
        ...
        RelativePath = maps:get(<<"path">>, Data1, undefined),

        {Path, Data2} = case {maps:get(<<"name">>, Data1, undefined), RelativePath} of
            {name_placeholder, DirPath} ->
                Name = str_utils:rand_hex(10),
                api_test_memory:set(MemRef, name, Name),
                api_test_memory:set(MemRef, path, DirPath),
                {<<DirPath/binary, Name/binary>>, maps:without([<<"name">>, <<"path">>], Data1)};
            {undefined, _} ->
                Name = str_utils:rand_hex(10),
                api_test_memory:set(MemRef, name, Name),
                api_test_memory:set(MemRef, path, undefined),
                {Name, maps:without([<<"name">>, <<"path">>], Data1)};
            {Name, DirPath} ->
                api_test_memory:set(MemRef, name, Name),
                api_test_memory:set(MemRef, path, DirPath),
                Data7 = maps:without([<<"name">>, <<"path">>], Data1),
                {str_utils:join_as_binaries([DirPath, Name], <<"/">>), Data7}
        end,
        {Body, Data3} = utils:ensure_defined(maps:take(body, Data2), error, {<<>>, Data2}),

        #rest_args{
            method = put,
            path = http_utils:append_url_parameters(<<"data/", ParentId/binary, "/path/", Path/binary>>, Data3),
            body = Body
        }
    end.
