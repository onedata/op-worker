...
-export([
    ...
    hardlinks_test/4,
    first_reading_and_writing_via_hardlink_test/4,
    create_hardlink_to_hardlink_test/4,
...
hardlinks_test(Config0, User, {SyncNodes, ProxyNodes, ProxyNodesWritten0, NodesOfProvider}, Attempts) ->
    ...
    ?assertEqual({error, not_found}, rpc:call(Worker2, times, get, [FileUuid])),
    ?assertEqual({error, not_found}, rpc:call(Worker2, file_location, get_local, [FileUuid])),
    ?assertEqual({error, not_found}, rpc:call(Worker1, times, get, [FileUuid]), Attempts),
    ?assertEqual({error, not_found}, rpc:call(Worker1, file_location, get_local, [FileUuid]), Attempts).

first_reading_and_writing_via_hardlink_test(Config, User, {SyncNodes, ProxyNodes, ProxyNodesWritten}, Attempts) ->
    first_reading_and_writing_via_hardlink_test(Config, User, {SyncNodes, ProxyNodes, ProxyNodesWritten, 1}, Attempts);
first_reading_and_writing_via_hardlink_test(Config0, User, {SyncNodes, ProxyNodes, ProxyNodesWritten0, NodesOfProvider}, Attempts) ->
    Config = extend_config(Config0, User, {SyncNodes, ProxyNodes, ProxyNodesWritten0, NodesOfProvider}, Attempts),
    SessId = ?config(session, Config),
    SpaceName = ?config(space_name, Config),
    Worker1 = ?config(worker1, Config),
...
create_hardlink_to_hardlink_test(Config, User, {SyncNodes, ProxyNodes, ProxyNodesWritten}, Attempts) ->
    create_hardlink_to_hardlink_test(Config, User, {SyncNodes, ProxyNodes, ProxyNodesWritten, 1}, Attempts);
create_hardlink_to_hardlink_test(Config0, User, {SyncNodes, ProxyNodes, ProxyNodesWritten0, NodesOfProvider}, Attempts) ->
    Config = extend_config(Config0, User, {SyncNodes, ProxyNodes, ProxyNodesWritten0, NodesOfProvider}, Attempts),
    SessId = ?config(session, Config),
    SpaceName = ?config(space_name, Config),
    Worker1 = ?config(worker1, Config),
...
%%%===================================================================
%%% Internal functions for hardlinks testing
%%%===================================================================
...
verify_hardlink_attrs(HardlinkName, HardlinkAttr, FileAttr, SpaceGuid) ->
    ...
    ?assertEqual(?HARDLINK_TYPE, HardlinkAttr#file_attr.type),
...
verify_hardlink_times_and_location_documents(Worker1, Worker2, FileUuid, HardlinkUuid) ->
    ...
    ?assertMatch({ok, _}, rpc:call(Worker1, times, get, [FileUuid])),
    {ok, #document{key = FileLocation1Uuid}} = ?assertMatch({ok, _}, rpc:call(Worker1, file_location, get_local, [FileUuid])),
    ?assertMatch({ok, _}, rpc:call(Worker2, times, get, [FileUuid])),
    {ok, #document{key = FileLocation2Uuid}} = ?assertMatch({ok, _}, rpc:call(Worker2, file_location, get_local, [FileUuid])),

    % Check if times and file_location documents are returned for hardlink
    ?assertMatch({ok, #document{key = FileUuid}}, rpc:call(Worker1, times, get, [HardlinkUuid])),
    ?assertMatch({ok, #document{key = FileLocation1Uuid}}, rpc:call(Worker1, file_location, get_local, [HardlinkUuid])),
    ?assertMatch({ok, #document{key = FileUuid}}, rpc:call(Worker2, times, get, [HardlinkUuid])),
    ?assertMatch({ok, #document{key = FileLocation2Uuid}}, rpc:call(Worker2, file_location, get_local, [HardlinkUuid])),
