...
%% loop in multi_provider_file_ops_test_base. An open error counts as a failed
%% attempt rather than aborting the retries.
-define(assertRead(Worker, SessId, FilePath, Offset, ExpectedData, Attempts), (
    fun(__Worker, __SessId, __FilePath, __Offset, __ExpectedData, __Attempts) ->
        ?assertEqual({ok, __ExpectedData}, case lfm_proxy:open(__Worker, __SessId, {path, __FilePath}, read) of
            {ok, __H} ->
                try
