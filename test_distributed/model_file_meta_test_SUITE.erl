...
list_test(Config) ->
    ...
    ?assertMatch({ok, [#child_link_uuid{name = <<"f1">>}, #child_link_uuid{name = <<"f2">>}, #child_link_uuid{name = <<"f3">>}], #{}},
        rpc:call(Worker1, file_meta, list_children, [{path, <<"/Space 1/list_test_d1">>}, 0, 100])),

    ?assertMatch({ok, [#child_link_uuid{name = <<"f1">>}, #child_link_uuid{name = <<"f2">>}, #child_link_uuid{name = <<"f3">>}], #{token := _}},
        rpc:call(Worker1, file_meta, list_children, [{path, <<"/Space 1/list_test_d1">>}, 100])),

    {ok, _, #{token := T1}} = ?assertMatch({ok, [#child_link_uuid{name = <<"f1">>}], #{token := _}},
        rpc:call(Worker1, file_meta, list_children, [{path, <<"/Space 1/list_test_d1">>}, 1])),
    {ok, _, #{token := T2}} = ?assertMatch({ok, [#child_link_uuid{name = <<"f2">>}], #{token := _}},
        rpc:call(Worker1, file_meta, list_children, [{path, <<"/Space 1/list_test_d1">>}, 0, 1, T1])),
    ?assertMatch({ok, [#child_link_uuid{name = <<"f3">>}], #{token := _}},
        rpc:call(Worker1, file_meta, list_children_by_key, [{path, <<"/Space 1/list_test_d1">>}, <<>>, <<>>, 1, T2])),
    ...
    ?assertMatch({ok, [#child_link_uuid{name = <<"f2">>}], #{token := _}},
        rpc:call(Worker1, file_meta, list_children, [{path, <<"/Space 1/list_test_d1">>}, 1, 1, T3])),
    ...
    timer:sleep(timer:seconds(10)),
    ?assertMatch({ok, [#child_link_uuid{name = <<"f2">>}], #{token := _}},
        rpc:call(Worker1, file_meta, list_children_by_key, [{path, <<"/Space 1/list_test_d1">>}, LN, LT, 1, T4])),
    ...
    ?assertMatch({ok, [#child_link_uuid{name = <<"f02">>}], #{token := _}},
        rpc:call(Worker1, file_meta, list_children_by_key, [{path, <<"/Space 1/list_test_d1">>}, LN2, LT2, 1, T5])),
    ...
    ?assertMatch({ok, [#child_link_uuid{name = <<"f02">>}], #{token := _}},
        rpc:call(Worker1, file_meta, list_children_by_key, [{path, <<"/Space 1/list_test_d1">>}, LN3, LT3, 1, T6])),
    ok.
