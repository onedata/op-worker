...
%%%
%%% QoS management is based on two types of documents: qos_item and file_qos
%%% each of which is synchronized within space.
%%%
%%% The qos_item document contains information about single QoS requirement like
...
%%% Multiple qos_item documents can be created for the same file or directory.
...
%%% qos_item can be related with links tree containing ID of transfers
%%% scheduled to fulfill QoS after file or file in directory has been changed.
%%% QoS requirement is fulfilled when:
%%% - there is no active traverse task in qos_item document
%%% - status is set to fulfilled in qos_item document
%%% - there is no active transfers in links tree
%%%
%%% The file_qos item contains aggregated information about QoS defined for file
%%% or directory. It contains list of qos_item IDs and mapping that maps storage_id
%%% to list of qos_item IDs that requires file replica on this storage. Each file
%%% or directory can be related with at most one such document. It is created/updated
%%% in one of following cases:
%%% - new qos_item document is created for file or directory. In this case
%%%   target storages are chosen according to QoS expression and number of
%%%   required replicas defined in qos_item document. Then file_qos document is
%%%   updated - qos_item ID is added to QoS list and target storages mapping,
...
%%%   qos_item ID and all chosen target storages (not only the ones that
%%%   differs from parent)
