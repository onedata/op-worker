...
-record(node_state, {node_type = worker, ccm_con_status = not_connected, state_num = 0, callbacks_num = 0, dispatcher_state = ok, callbacks_state = ok, memory_and_network_info = [], callbacks = [], simple_caches = []}).
...
-record(cm_state, {nodes = [], workers = [], dispatcher_maps = [], state_num = 1, callbacks_num = 1, cluster_check_num = 0, state_loaded = false}).
...
%% load balancing).
-record(host_state, {plug_in = non, request_map = non, dispatcher_request_map = non, dispatcher_request_map_ok = true, sub_procs = [], plug_in_state = [], load_info = [], current_seq_job = none, seq_queue = []}).
