...
RELEASE_VERSION ?= $(shell grep -o '{vsn, "[^"]*"' src/op_worker.app.src | cut -d'"' -f2)
...
.PHONY: template
template:
	sed -e "s/{service_build_version, \".*\"}/{service_build_version, \"${BUILD_VERSION}\"}/" \
	    -e "s/{service_release_version, \".*\"}/{service_release_version, \"${RELEASE_VERSION}\"}/" \
	    ./rel/vars.config.template > ./rel/vars.config
...
codetag-tracker:
	@./bamboos/scripts/run-with-surefire-report.py \
		...
		--excluded-dirs=node_package,gpb,helpers,gen_server_mock,rrdtool,rtransfer_link,locks,codetag_tracker_results,ezlib \
