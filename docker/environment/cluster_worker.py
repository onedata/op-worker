...
def cluster_domain(cluster_instance, uid):
    """Formats domain for a cluster."""
    return common.format_hostname(cluster_instance, uid)


def worker_hostname(node_name, cluster_instance, uid):
    """Formats hostname for a docker hosting cluster_worker.
    NOTE: Hostnames are also used as docker names!
    """
    return common.format_hostname([node_name, cluster_instance], uid)


def worker_erl_node_name(node_name, cluster_instance, uid):
    """Formats erlang node name for a vm on cluster_worker docker.
    """
    hostname = worker_hostname(node_name, cluster_instance, uid)
    return common.format_erl_node_name('worker', hostname)


def _tweak_config(config, name, cluster_instance, uid):
    cfg = copy.deepcopy(config)
    cfg['nodes'] = {'node': cfg['nodes'][name]}

    sys_config = cfg['nodes']['node']['sys.config']
    sys_config['ccm_nodes'] = [
        provider_ccm.ccm_erl_node_name(n, cluster_instance, uid) for n in
        sys_config['ccm_nodes']]
    # Set the cluster domain (needed for nodes to start)
    sys_config['cluster_domain'] = cluster_domain(cluster_instance, uid)
    ...
    vm_args['name'] = worker_erl_node_name(name, cluster_instance, uid)
...
/root/bin/node/bin/cluster_worker console'''
    ...
    return container, {
        ...
        'cluster_worker_nodes': [node_name]
    }
...
def up(image, bindir, dns_server, uid, config_path, logdir=None):
    ...
    input_dir = config['dirs_config']['cluster_worker']['input_dir']
    dns_servers, output = dns.maybe_start(dns_server, uid)

    # Workers of every cluster are started together
    for cluster_instance in config['cluster_domains']:
        os_config = config['cluster_domains'][cluster_instance]['os_config']
        gen_dev_cfg = {
            'config': {
                'input_dir': input_dir,
                'target_dir': '/root/bin'
            },
            'nodes': config['cluster_domains'][cluster_instance]['cluster_worker'],
            'db_driver': _db_driver(config['cluster_domains'][cluster_instance]),
        ...
        if db_driver == 'riak':
            db_node_mappings, db_out = _riak_up(cluster_instance, all_db_nodes, dns_servers, uid)
        elif db_driver == 'couchbase':
            db_node_mappings, db_out = _couchbase_up(cluster_instance, all_db_nodes, dns_servers, uid)
        else:
        ...
        domains = {
            'domains': {
                cluster_domain(cluster_instance, uid): {
                    'ns': worker_ips,
                    'a': []
                }
            }
        ...
        create_storages(config['os_configs'][os_config]['storages'],
                        ...
                        config['cluster_domains'][cluster_instance]['cluster_worker'],
                        bindir)
