...
NAME_ARG=`egrep '^\-name' $RUNNER_ETC_DIR/autogenerated.vm.args 2> /dev/null`
if [ -z "$NAME_ARG" ]; then
...
COOKIE_ARG=`grep '^\-setcookie' $RUNNER_ETC_DIR/autogenerated.vm.args 2> /dev/null`
...
NET_TICKTIME_ARG=`grep '^\-kernel net_ticktime' $RUNNER_DATA_DIR/vm.args 2> /dev/null`
...
check_config() {
    if [ -z "$CUTTLEFISH" ]; then
        ...
        CONFIG_ARGS=" -config $RUNNER_DATA_DIR/app.config -args_file $RUNNER_DATA_DIR/vm.args -vm_args $RUNNER_DATA_DIR/vm.args "
    else
