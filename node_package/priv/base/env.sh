...
RUN_DIR="{{runner_run_dir}}" # Defaults to /var/run
...
check_config() {
    if [ -z "$CUTTLEFISH" ]; then
        ...
        CONFIG_ARGS=" -config $RUNNER_DATA_DIR/app.config -args_file $RUNNER_ETC_DIR/vm.args -vm_args $RUNNER_ETC_DIR/vm.args "
    else
    ...
    CONFIG_ARGS="$CONFIG_ARGS -config $RUNNER_ETC_DIR/autogenerated.config "
