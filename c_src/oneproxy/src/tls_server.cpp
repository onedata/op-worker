...
namespace proxy {
...

    {
        ...
        context_.use_private_key_file(cert_path,
    ...
    void tls_server::start_accept()
    {
        ...
        new_session->sslsocket().set_verify_callback(
            boost::bind(&tls2tcp_session::handle_verify_certificate,
                        new_session.get(), _1, _2));

        acceptor_.async_accept(new_session->socket(),
                               boost::bind(&tls_server::handle_accept, shared_from_this(),
                                           new_session,
                                           boost::asio::placeholders::error));
    }

    void tls_server::handle_accept(boost::shared_ptr
                                   <tls2tcp_session> new_session,
    ...
    void tls_server::load_certs()
    {
        ...
        LOG(INFO) << "Loaded " << ca_certs_.size() << " CAs and "
                  << crls_.size() << " CRLs";
    }
